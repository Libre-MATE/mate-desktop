...
enum { CHANGED, TRANSITIONED, N_SIGNALS };

static guint signals[N_SIGNALS] = {0};

G_DEFINE_TYPE(MateBG, mate_bg, G_TYPE_OBJECT)

static cairo_surface_t *make_root_pixmap(GdkWindow *window, gint width,
                                         gint height);
...
static GdkPixbuf *pixbuf_scale_to_fit(GdkPixbuf *src, int max_width,
                                      int max_height);
static GdkPixbuf *pixbuf_scale_to_min(GdkPixbuf *src, int min_width,
                                      int min_height);

static void pixbuf_draw_gradient(GdkPixbuf *pixbuf, gboolean horizontal,
                                 GdkRGBA *c1, GdkRGBA *c2, GdkRectangle *rect);

static void pixbuf_tile(GdkPixbuf *src, GdkPixbuf *dest);
static void pixbuf_blend(GdkPixbuf *src, GdkPixbuf *dest, int src_x, int src_y,
                         int width, int height, int dest_x, int dest_y,
                         double alpha);

/* Thumbnail utilities */
static GdkPixbuf *create_thumbnail_for_filename(
    MateDesktopThumbnailFactory *factory, const char *filename);
static gboolean get_thumb_annotations(GdkPixbuf *thumb, int *orig_width,
                                      int *orig_height);

/* Cache */
static GdkPixbuf *get_pixbuf_for_size(MateBG *bg, gint num_monitor, int width,
                                      int height);
static void clear_cache(MateBG *bg);
static gboolean is_different(MateBG *bg, const char *filename);
...
static SlideShow *get_as_slideshow(MateBG *bg, const char *filename);
static Slide *get_current_slide(SlideShow *show, double *alpha);
static gboolean slideshow_has_multiple_sizes(SlideShow *show);

static SlideShow *read_slideshow_file(const char *filename, GError **err);
static SlideShow *slideshow_ref(SlideShow *show);
static void slideshow_unref(SlideShow *show);

static FileSize *find_best_size(GSList *sizes, gint width, gint height);

static void color_from_string(const char *string, GdkRGBA *colorp) {
  if (!string || *string == '\0' || !gdk_rgba_parse(colorp, string))
    gdk_rgba_parse(colorp, "#000000"); /* If all else fails use black */
}

static char *color_to_string(const GdkRGBA *color) {
  return g_strdup_printf("#%02x%02x%02x", ((guint)(color->red * 65535)) >> 8,
                         ((guint)(color->green * 65535)) >> 8,
                         ((guint)(color->blue * 65535)) >> 8);
}

static gboolean do_changed(MateBG *bg) {
  bg->changed_id = 0;
...
static void queue_changed(MateBG *bg) {
  if (bg->changed_id > 0) {
    g_source_remove(bg->changed_id);
  }

  bg->changed_id = g_timeout_add_full(G_PRIORITY_LOW, 100,
                                      (GSourceFunc)do_changed, bg, NULL);
}

static gboolean do_transitioned(MateBG *bg) {
...
static void queue_transitioned(MateBG *bg) {
  if (bg->transitioned_id > 0) {
    g_source_remove(bg->transitioned_id);
  }

  bg->transitioned_id = g_timeout_add_full(
      G_PRIORITY_LOW, 100, (GSourceFunc)do_transitioned, bg, NULL);
}
...
void mate_bg_load_from_system_gsettings(MateBG *bg, GSettings *settings,
...
void mate_bg_load_from_gsettings(MateBG *bg, GSettings *settings) {
  ...
  bg->is_enabled =
      g_settings_get_boolean(settings, MATE_BG_KEY_DRAW_BACKGROUND);
...
void mate_bg_save_to_gsettings(MateBG *bg, GSettings *settings) {
  gchar *primary;
  gchar *secondary;
...
static void mate_bg_init(MateBG *bg) {}

static void mate_bg_dispose(GObject *object) {
...
static void mate_bg_finalize(GObject *object) {
  MateBG *bg = MATE_BG(object);
...
static void mate_bg_class_init(MateBGClass *klass) {
  ...
  signals[CHANGED] = g_signal_new(
      "changed", G_OBJECT_CLASS_TYPE(object_class), G_SIGNAL_RUN_LAST, 0, NULL,
      NULL, g_cclosure_marshal_VOID__VOID, G_TYPE_NONE, 0);

  signals[TRANSITIONED] = g_signal_new(
      "transitioned", G_OBJECT_CLASS_TYPE(object_class), G_SIGNAL_RUN_LAST, 0,
      NULL, NULL, g_cclosure_marshal_VOID__VOID, G_TYPE_NONE, 0);
}

MateBG *mate_bg_new(void) { return g_object_new(MATE_TYPE_BG, NULL); }

void mate_bg_set_color(MateBG *bg, MateBGColorType type, GdkRGBA *primary,
                       GdkRGBA *secondary) {
  g_return_if_fail(bg != NULL);
  g_return_if_fail(primary != NULL);

  if (bg->color_type != type || !gdk_rgba_equal(&bg->primary, primary) ||
...
void mate_bg_set_placement(MateBG *bg, MateBGPlacement placement) {
  g_return_if_fail(bg != NULL);
...
void mate_bg_get_color(MateBG *bg, MateBGColorType *type, GdkRGBA *primary,
                       GdkRGBA *secondary) {
  g_return_if_fail(bg != NULL);

  if (type) *type = bg->color_type;

  if (primary) *primary = bg->primary;

  if (secondary) *secondary = bg->secondary;
}

void mate_bg_set_draw_background(MateBG *bg, gboolean draw_background) {
  g_return_if_fail(bg != NULL);
...
static inline gchar *get_wallpaper_cache_dir(void) {
  return g_build_filename(g_get_user_cache_dir(), MATE_BG_CACHE_DIR, NULL);
}

static inline gchar *get_wallpaper_cache_prefix_name(gint num_monitor,
                                                     MateBGPlacement placement,
                                                     gint width, gint height) {
  return g_strdup_printf("%i_%i_%i_%i", num_monitor, (gint)placement, width,
                         height);
}

static char *get_wallpaper_cache_filename(const char *filename,
                                          gint num_monitor,
                                          MateBGPlacement placement, gint width,
                                          gint height) {
  ...
  md5_filename = g_compute_checksum_for_data(
      G_CHECKSUM_MD5, (const guchar *)filename, strlen(filename));
  cache_prefix_name =
      get_wallpaper_cache_prefix_name(num_monitor, placement, width, height);
...
static void cleanup_cache_for_monitor(gchar *cache_dir, gint num_monitor) {
  GDir *g_cache_dir;
...
static gboolean cache_file_is_valid(const char *filename,
                                    const char *cache_filename) {
  if (!g_file_test(cache_filename, G_FILE_TEST_IS_REGULAR)) return FALSE;

  return (get_mtime(filename) < get_mtime(cache_filename));
}

static void refresh_cache_file(MateBG *bg, GdkPixbuf *new_pixbuf,
  ...
  if (!cache_file_is_valid(bg->filename, cache_filename)) {
    ...
    if (format != NULL) {
      ...
      if (strcmp(format_name, "jpeg") == 0)
        gdk_pixbuf_save(new_pixbuf, cache_filename, format_name, NULL,
                        "quality", "100", NULL);
      else
...
static void file_changed(GFileMonitor *file_monitor, GFile *child,
...
void mate_bg_set_filename(MateBG *bg, const char *filename) {
  g_return_if_fail(bg != NULL);
  ...
  if (is_different(bg, filename)) {
    ...
    if (bg->filename) {
      ...
      g_signal_connect(bg->file_monitor, "changed", G_CALLBACK(file_changed),
                       bg);
  ...
  switch (bg->color_type) {
    case MATE_BG_COLOR_SOLID:
      ...
      pixel = ((guint)(bg->primary.red * 0xff) << 24) |
              ...
              ((guint)(bg->primary.blue * 0xff) << 8) | (0xff);
...
static void draw_color(MateBG *bg, GdkPixbuf *dest) {
  GdkRectangle rect;
...
static void draw_color_each_monitor(MateBG *bg, GdkPixbuf *dest,
                                    GdkScreen *screen) {
  GdkDisplay *display;
  GdkRectangle rect;
  gint num_monitors;
...
static GdkPixbuf *pixbuf_clip_to_fit(GdkPixbuf *src, int max_width,
                                     int max_height) {
  ...
  pixbuf = gdk_pixbuf_new(GDK_COLORSPACE_RGB, gdk_pixbuf_get_has_alpha(src), 8,
                          w, h);

  src_x = (src_width - w) / 2;
  src_y = (src_height - h) / 2;
  gdk_pixbuf_copy_area(src, src_x, src_y, w, h, pixbuf, 0, 0);
  return pixbuf;
}

static GdkPixbuf *get_scaled_pixbuf(MateBGPlacement placement,
                                    GdkPixbuf *pixbuf, int width, int height,
...
static void draw_image_area(MateBG *bg, gint num_monitor, GdkPixbuf *pixbuf,
                            GdkPixbuf *dest, GdkRectangle *area) {
  ...
  scaled = get_scaled_pixbuf(bg->placement, pixbuf, dest_width, dest_height, &x,
                             &y, &w, &h);
...
static void draw_image_for_thumb(MateBG *bg, GdkPixbuf *pixbuf,
                                 GdkPixbuf *dest) {
  GdkRectangle rect;
...
static void draw_once(MateBG *bg, GdkPixbuf *dest, gboolean is_root) {
  GdkRectangle rect;
  GdkPixbuf *pixbuf;
  gint monitor;
...
static void draw_each_monitor(MateBG *bg, GdkPixbuf *dest, GdkScreen *screen) {
  GdkDisplay *display;
...
void mate_bg_draw(MateBG *bg, GdkPixbuf *dest, GdkScreen *screen,
                  gboolean is_root) {
  if (!bg) return;
...
static void mate_bg_get_pixmap_size(MateBG *bg, int width, int height,
                                    int *pixmap_width, int *pixmap_height) {
  int dummy;

  if (!pixmap_width) pixmap_width = &dummy;
  if (!pixmap_height) pixmap_height = &dummy;
 ...
 * who created it.
 **/
cairo_surface_t *mate_bg_create_surface(MateBG *bg, GdkWindow *window,
                                        int width, int height, gboolean root) {
  return mate_bg_create_surface_scale(bg, window, width, height, 1, root);
}
 ...
 * @root:
 *
 * Create a scaled surface that can be set as background for @window. If @root
 *is TRUE, the surface created will be created by a temporary X server
 *connection so that if someone calls XKillClient on it, it won't affect the
 *application who created it.
 **/
cairo_surface_t *mate_bg_create_surface_scale(MateBG *bg, GdkWindow *window,
  ...
  if (bg->pixbuf_cache && (gdk_pixbuf_get_width(bg->pixbuf_cache) != width ||
 ...
 */
gboolean mate_bg_is_dark(MateBG *bg, int width, int height) {
  GdkRGBA color;
  int intensity;
  GdkPixbuf *pixbuf;
...
static cairo_surface_t *make_root_pixmap(GdkWindow *window, gint width,
   ...
   * applications will try to kill it with XKillClient later when changing
   * pixmap
   */
  ...
  surface = cairo_xlib_surface_create(
      ...
      GDK_VISUAL_XVISUAL(gdk_screen_get_system_visual(screen)), width, height);

  return surface;
}

static gboolean get_original_size(const char *filename, int *orig_width,
                                  int *orig_height) {
  gboolean result;
...
static const char *get_filename_for_size(MateBG *bg, gint best_width,
                                         gint best_height) {
  SlideShow *show;
  Slide *slide;
  FileSize *size;

  if (!bg->filename) return NULL;
...
gboolean mate_bg_get_image_size(MateBG *bg,
                                MateDesktopThumbnailFactory *factory,
                                int best_width, int best_height, int *width,
                                int *height) {
  GdkPixbuf *thumb;
  ...
  if (!bg->filename) return FALSE;

  filename = get_filename_for_size(bg, best_width, best_height);
  thumb = create_thumbnail_for_filename(factory, filename);
  if (thumb) {
    if (get_thumb_annotations(thumb, width, height)) result = TRUE;

    g_object_unref(thumb);
  }

  if (!result) {
    if (get_original_size(filename, width, height)) result = TRUE;
  }

  return result;
}

static double fit_factor(int from_width, int from_height, int to_width,
                         int to_height) {
...
GdkPixbuf *mate_bg_create_thumbnail(MateBG *bg,
  ...
  result =
      gdk_pixbuf_new(GDK_COLORSPACE_RGB, FALSE, 8, dest_width, dest_height);
...
cairo_surface_t *mate_bg_get_surface_from_root(GdkScreen *screen) {
  ...
  result = XGetWindowProperty(display, RootWindow(display, screen_num),
                              gdk_x11_get_xatom_by_name("_XROOTPMAP_ID"), 0L,
                              1L, False, XA_PIXMAP, &type, &format, &nitems,
                              &bytes_after, &data);
  surface = NULL;
  source_pixmap = NULL;
  ...
  if (data != NULL) {
    ...
    if (XGetGeometry(GDK_SCREEN_XDISPLAY(screen), xpixmap, &root_return, &x_ret,
  ...
  if (surface == NULL) {
    surface = gdk_window_create_similar_surface(
        gdk_screen_get_root_window(screen), CAIRO_CONTENT_COLOR, width, height);
  }

  if (source_pixmap != NULL) cairo_surface_destroy(source_pixmap);
 ...
 */
static void mate_bg_set_root_pixmap_id(GdkScreen *screen, Display *display,
                                       Pixmap xpixmap) {
  ...
  if (XInternAtoms(display, atom_names, G_N_ELEMENTS(atom_names), True,
    ...
    if (data_root != NULL && result == Success && type == XA_PIXMAP &&
        format == 32 && nitems == 1) {
      result = XGetWindowProperty(display, xroot, atoms[1], 0L, 1L, False,
                                  AnyPropertyType, &type, &format, &nitems,
                                  &after, &data_esetroot);

      if (data_esetroot != NULL && result == Success && type == XA_PIXMAP &&
          format == 32 && nitems == 1) {
   ...
   * This method is to avoid multiple round-trips to Xserver
   */
  if (!XInternAtoms(display, atom_names, G_N_ELEMENTS(atom_names), False,
                    atoms) ||
      atoms[0] == None || atoms[1] == None) {
    g_warning(
        "Could not create atoms needed to set root pixmap id/properties.\n");
    return;
  }

  /* Set new _XROOTMAP_ID and ESETROOT_PMAP_ID properties */
  XChangeProperty(display, xroot, atoms[0], XA_PIXMAP, 32, PropModeReplace,
                  (unsigned char *)&xpixmap, 1);

  XChangeProperty(display, xroot, atoms[1], XA_PIXMAP, 32, PropModeReplace,
                  (unsigned char *)&xpixmap, 1);
}
 ...
 **/
void mate_bg_set_surface_as_root(GdkScreen *screen, cairo_surface_t *surface) {
 ...
 **/
MateBGCrossfade *mate_bg_set_surface_as_root_with_crossfade(
...
static Slide *get_current_slide(SlideShow *show, double *alpha) {
  ...
  for (list = show->slides->head; list != NULL; list = list->next) {
    ...
    if (elapsed + slide->duration > delta) {
      if (alpha) *alpha = (delta - elapsed) / (double)slide->duration;
      return slide;
    }
...
static GdkPixbuf *blend(GdkPixbuf *p1, GdkPixbuf *p2, double alpha) {
  ...
  if (gdk_pixbuf_get_width(p2) != gdk_pixbuf_get_width(p1) ||
    ...
    tmp =
        gdk_pixbuf_scale_simple(p2, gdk_pixbuf_get_width(p1),
                                gdk_pixbuf_get_height(p1), GDK_INTERP_BILINEAR);
  } else {
    tmp = g_object_ref(p2);
  }
...
typedef enum { PIXBUF, SLIDESHOW, THUMBNAIL } FileType;
...
static void file_cache_entry_delete(FileCacheEntry *ent) {
  g_free(ent->filename);
...
static void bound_cache(MateBG *bg) {
...
static const FileCacheEntry *file_cache_lookup(MateBG *bg, FileType type,
                                               const char *filename) {
  GList *list;
  ...
  for (list = bg->file_cache; list != NULL; list = list->next) {
    ...
    if (ent && ent->type == type && strcmp(ent->filename, filename) == 0) {
      return ent;
    }
  }
...
static FileCacheEntry *file_cache_entry_new(MateBG *bg, FileType type,
                                            const char *filename) {
...
static void file_cache_add_pixbuf(MateBG *bg, const char *filename,
                                  GdkPixbuf *pixbuf) {
  FileCacheEntry *ent = file_cache_entry_new(bg, PIXBUF, filename);
  ent->u.pixbuf = g_object_ref(pixbuf);
}

static void file_cache_add_thumbnail(MateBG *bg, const char *filename,
                                     GdkPixbuf *pixbuf) {
  FileCacheEntry *ent = file_cache_entry_new(bg, THUMBNAIL, filename);
  ent->u.thumbnail = g_object_ref(pixbuf);
}

static void file_cache_add_slide_show(MateBG *bg, const char *filename,
                                      SlideShow *show) {
  FileCacheEntry *ent = file_cache_entry_new(bg, SLIDESHOW, filename);
  ent->u.slideshow = slideshow_ref(show);
}

static GdkPixbuf *load_from_cache_file(MateBG *bg, const char *filename,
                                       gint num_monitor, gint best_width,
                                       gint best_height) {
  GdkPixbuf *pixbuf = NULL;
  gchar *cache_filename;

  cache_filename = get_wallpaper_cache_filename(
      filename, num_monitor, bg->placement, best_width, best_height);
...
static GdkPixbuf *get_as_pixbuf_for_size(MateBG *bg, const char *filename,
                                         gint monitor, gint best_width,
                                         gint best_height) {
  ...
  if ((ent = file_cache_lookup(bg, PIXBUF, filename))) {
    ...
    if (monitor != -1)
      pixbuf =
          load_from_cache_file(bg, filename, monitor, best_width, best_height);
    ...
    if (!pixbuf) {
      ...
      if (g_strcmp0(tmp, "svg") == 0 && (best_width > 0 && best_height > 0) &&
        ...
        pixbuf = gdk_pixbuf_new_from_file_at_size(filename, best_width,
                                                  best_height, NULL);
      } else {
...
static SlideShow *get_as_slideshow(MateBG *bg, const char *filename) {
  ...
  if ((ent = file_cache_lookup(bg, SLIDESHOW, filename))) {
    ...
    if (show) file_cache_add_slide_show(bg, filename, show);

    return show;
  }
}

static GdkPixbuf *get_as_thumbnail(MateBG *bg,
                                   MateDesktopThumbnailFactory *factory,
                                   const char *filename) {
  const FileCacheEntry *ent;
  if ((ent = file_cache_lookup(bg, THUMBNAIL, filename))) {
    ...
    if (thumb) file_cache_add_thumbnail(bg, filename, thumb);

    return thumb;
  }
}

static gboolean blow_expensive_caches(gpointer data) {
  MateBG *bg = data;
  GList *list;
  ...
  if (bg->file_cache) {
    for (list = bg->file_cache; list != NULL; list = list->next) {
      ...
      if (ent->type == PIXBUF) {
        file_cache_entry_delete(ent);
        bg->file_cache = g_list_delete_link(bg->file_cache, list);
      }
    }
  }
...
static void blow_expensive_caches_in_idle(MateBG *bg) {
  if (bg->blow_caches_id == 0) {
    bg->blow_caches_id = g_idle_add(blow_expensive_caches, bg);
  }
}
...
static double get_slide_timeout(Slide *slide) {
  double timeout;
  if (slide->fixed) {
...
static GdkPixbuf *scale_thumbnail(MateBGPlacement placement,
                                  const char *filename, GdkPixbuf *thumb,
                                  GdkScreen *screen, int dest_width,
                                  int dest_height) {
  int o_width;
  int o_height;
  ...
  if (get_thumb_annotations(thumb, &o_width, &o_height) ||
    ...
    double screen_to_dest =
        fit_factor(scr_width, scr_height, dest_width, dest_height);
    double thumb_to_orig =
        fit_factor(thumb_width, thumb_height, o_width, o_height);
...
static GdkPixbuf *create_img_thumbnail(MateBG *bg,
  ...
  if (bg->filename) {
    ...
    if (thumb) {
      GdkPixbuf *result;
      result = scale_thumbnail(bg->placement, bg->filename, thumb, screen,
      ...
      if (show) {
        ...
        if (slide->fixed) {
          ...
          if (tmp) {
            thumb = scale_thumbnail(bg->placement, fs->file, tmp, screen,
                                    dest_width, dest_height);
            g_object_unref(tmp);
          }
        } else {
          ...
          if (p1 && p2) {
            ...
            thumb1 = scale_thumbnail(bg->placement, fs1->file, p1, screen,
                                     dest_width, dest_height);

            thumb2 = scale_thumbnail(bg->placement, fs2->file, p2, screen,
                                     dest_width, dest_height);
...
static FileSize *find_best_size(GSList *sizes, gint width, gint height) {
  ...
  for (pass = 0; pass < 2; pass++) {
    for (s = sizes; s; s = s->next) {
      ...
      if (pass == 0 && (size->width < width || size->height < height)) continue;
...
static GdkPixbuf *get_pixbuf_for_size(MateBG *bg, gint monitor, gint best_width,
                                      gint best_height) {
  ...
  if (bg->pixbuf_cache) {
    ...
    hit_cache = 0.2 > fabs((best_width / (double)best_height) -
                           (width / (double)height));
    if (!hit_cache) {
      g_object_unref(bg->pixbuf_cache);
      bg->pixbuf_cache = NULL;
  ...
  if (!hit_cache && bg->filename) {
    ...
    if (!bg->pixbuf_cache) {
      ...
      if (show) {
        ...
        if (slide->fixed) {
          FileSize *size =
              find_best_size(slide->file1, best_width, best_height);
          bg->pixbuf_cache = get_as_pixbuf_for_size(bg, size->file, monitor,
                                                    best_width, best_height);
        } else {
          FileSize *size;
          GdkPixbuf *p1, *p2;
          ...
          size = find_best_size(slide->file1, best_width, best_height);
          p1 = get_as_pixbuf_for_size(bg, size->file, monitor, best_width,
                                      best_height);

          size = find_best_size(slide->file2, best_width, best_height);
          p2 = get_as_pixbuf_for_size(bg, size->file, monitor, best_width,
                                      best_height);

          if (p1 && p2) bg->pixbuf_cache = blend(p1, p2, alpha);
          if (p1) g_object_unref(p1);
          if (p2) g_object_unref(p2);
        }
  ...
  if (bg->pixbuf_cache) g_object_ref(bg->pixbuf_cache);
...
static gboolean is_different(MateBG *bg, const char *filename) {
  if (!filename && bg->filename) {
    ...
    if (get_mtime(filename) != bg->file_mtime) return TRUE;

    if (strcmp(filename, bg->filename) != 0) return TRUE;
...
static GdkPixbuf *pixbuf_scale_to_min(GdkPixbuf *src, int min_width,
  ...
  /* crop the result */
  gdk_pixbuf_scale(src, dest, 0, 0, min_width, min_height,
                   (new_width - min_width) / -2, (new_height - min_height) / -2,
                   factor, factor, GDK_INTERP_BILINEAR);
  return dest;
}
...
static void pixbuf_draw_gradient(GdkPixbuf *pixbuf, gboolean horizontal,
  ...
  width = rect->width;
  height = rect->height;
  dst = gdk_pixbuf_get_pixels(pixbuf) + rect->x * n_channels +
        rowstride * rect->y;
...
static void pixbuf_tile(GdkPixbuf *src, GdkPixbuf *dest) {
  ...
  for (y = 0; y < dest_height; y += tile_height) {
    for (x = 0; x < dest_width; x += tile_width) {
      pixbuf_blend(src, dest, 0, 0, tile_width, tile_height, x, y, 1.0);
    }
  }
}
...
static void handle_start_element(GMarkupParseContext *context,
                                 ...
                                 const gchar **attr_values, gpointer user_data,
                                 GError **err) {
  ...
  if (strcmp(name, "static") == 0 || strcmp(name, "transition") == 0) {
    ...
    if (strcmp(name, "static") == 0) slide->fixed = TRUE;
...
static void handle_end_element(GMarkupParseContext *context, const gchar *name,
...
static void handle_text(GMarkupParseContext *context, const gchar *text,
  ...
  if (stack_is(parser, "year", "starttime", "background", NULL)) {
    ...
    for (i = 0; text[i]; i++) {
      if (!g_ascii_isspace(text[i])) break;
    }
    if (text[i] == 0) return;
    fs = g_new(FileSize, 1);
    fs->width = -1;
    ...
    if (slide->file1->next != NULL) parser->has_multiple_sizes = TRUE;
  } else if (stack_is(parser, "size", "file", "static", "background", NULL) ||
             stack_is(parser, "size", "from", "transition", "background",
                      NULL)) {
    g_return_if_fail(slide != NULL);
    ...
    if (slide->file2->next != NULL) parser->has_multiple_sizes = TRUE;
...
static GdkPixbuf *create_thumbnail_for_filename(
  ...
  if (thumb) {
    ...
    if (orig) {
      ...
      mate_desktop_thumbnail_factory_save_thumbnail(factory, result, uri,
                                                    mtime);
    } else {
      mate_desktop_thumbnail_factory_create_failed_thumbnail(factory, uri,
                                                             mtime);
    }
  }
