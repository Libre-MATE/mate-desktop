...
void test_one_locale(const gchar *locale) {
  ...
  if (mate_parse_locale(locale, &language_code, &country_code, &codeset,
                        &modifier)) {
    ...
    if (mate_language_has_translations(language_code)) {
      printf(
          "[mate_parse_locale]:\tlang_code=%s, country_code=%s, code=%s, "
          "modifier=%s\n"
          "[code]:\t\t\tlang=%s, country=%s, Has translation\n",
          language_code, country_code, codeset, modifier, lang, country);
    } else {
      printf(
          "[mate_parse_locale]:\tlang_code=%s, country_code=%s, code=%s, "
          "modifier=%s\n"
