...
static gboolean parse_file_gmarkup(const gchar *file,
                                   const GMarkupParser *parser, gpointer data,
                                   GError **err);

typedef struct CrtcAssignment CrtcAssignment;

static gboolean crtc_assignment_apply(CrtcAssignment *assign, guint32 timestamp,
                                      GError **error);
...
enum { PROP_0, PROP_SCREEN, PROP_LAST };
...
static void handle_text(GMarkupParseContext *context, const gchar *text,
  ...
  if (stack_is(parser, "vendor", "output", "configuration", TOPLEVEL_ELEMENT,
               NULL)) {
    parser->output->priv->connected = TRUE;

    strncpy((gchar *)parser->output->priv->vendor, text, 3);
    parser->output->priv->vendor[3] = 0;
  } else if (stack_is(parser, "clone", "configuration", TOPLEVEL_ELEMENT,
                      NULL)) {
    if (strcmp(text, "yes") == 0) parser->configuration->priv->clone = TRUE;
  } else if (stack_is(parser, "product", "output", "configuration",
                      TOPLEVEL_ELEMENT, NULL)) {
    parser->output->priv->connected = TRUE;

    parser->output->priv->product = parse_int(text);
  } else if (stack_is(parser, "serial", "output", "configuration",
                      TOPLEVEL_ELEMENT, NULL)) {
    parser->output->priv->connected = TRUE;

    parser->output->priv->serial = parse_uint(text);
  } else if (stack_is(parser, "width", "output", "configuration",
                      TOPLEVEL_ELEMENT, NULL)) {
    parser->output->priv->on = TRUE;

    parser->output->priv->width = parse_int(text);
  } else if (stack_is(parser, "x", "output", "configuration", TOPLEVEL_ELEMENT,
                      NULL)) {
    parser->output->priv->on = TRUE;

    parser->output->priv->x = parse_int(text);
  } else if (stack_is(parser, "y", "output", "configuration", TOPLEVEL_ELEMENT,
                      NULL)) {
    parser->output->priv->on = TRUE;

    parser->output->priv->y = parse_int(text);
  } else if (stack_is(parser, "height", "output", "configuration",
                      TOPLEVEL_ELEMENT, NULL)) {
    parser->output->priv->on = TRUE;

    parser->output->priv->height = parse_int(text);
  } else if (stack_is(parser, "rate", "output", "configuration",
                      TOPLEVEL_ELEMENT, NULL)) {
    parser->output->priv->on = TRUE;

    parser->output->priv->rate = parse_int(text);
  } else if (stack_is(parser, "rotation", "output", "configuration",
                      TOPLEVEL_ELEMENT, NULL)) {
  ...
  } else if (stack_is(parser, "reflect_x", "output", "configuration",
                      TOPLEVEL_ELEMENT, NULL)) {
    if (strcmp(text, "yes") == 0) {
      parser->output->priv->rotation |= MATE_RR_REFLECT_X;
    }
  } else if (stack_is(parser, "reflect_y", "output", "configuration",
                      TOPLEVEL_ELEMENT, NULL)) {
    if (strcmp(text, "yes") == 0) {
      parser->output->priv->rotation |= MATE_RR_REFLECT_Y;
    }
  } else if (stack_is(parser, "primary", "output", "configuration",
                      TOPLEVEL_ELEMENT, NULL)) {
    if (strcmp(text, "yes") == 0) {
...
static void parser_free(Parser *parser) {
  ...
  if (parser->output) g_object_unref(parser->output);

  if (parser->configuration) g_object_unref(parser->configuration);
  ...
  for (list = parser->stack->head; list; list = list->next) g_free(list->data);
  g_queue_free(parser->stack);

  g_free(parser);
}

static MateRRConfig **configurations_read_from_file(const gchar *filename,
                                                    GError **error) {
...
static void mate_rr_config_init(MateRRConfig *self) {
...
static void mate_rr_config_set_property(GObject *gobject, guint property_id,
                                        const GValue *value,
                                        GParamSpec *property) {
...
static void mate_rr_config_finalize(GObject *gobject) {
  MateRRConfig *self = MATE_RR_CONFIG(gobject);
...
gboolean mate_rr_config_load_current(MateRRConfig *config, GError **error) {
  GPtrArray *a;
  ...
  for (i = 0; rr_outputs[i] != NULL; ++i) {
    ...
    if (!output->priv->connected) {
      ...
      if (edid_data) info = decode_edid(edid_data);
...
gboolean mate_rr_config_load_filename(MateRRConfig *result,
  ...
  if (filename == NULL) filename = mate_rr_config_get_intended_filename();
  ...
  if (configs) {
    ...
    for (i = 0; configs[i] != NULL; ++i) {
      if (mate_rr_config_match(configs[i], current)) {
        ...
        result->priv->outputs =
            (MateRROutputInfo **)g_ptr_array_free(array, FALSE);
    ...
    if (!found)
      g_set_error(error, MATE_RR_ERROR, MATE_RR_ERROR_NO_MATCHING_CONFIG,
                  _("none of the saved display configurations matched the "
...
static void mate_rr_config_class_init(MateRRConfigClass *klass) {
  ...
  g_object_class_install_property(
      gobject_class, PROP_SCREEN,
      g_param_spec_object("screen", "Screen",
                          "The MateRRScreen this config applies to",
                          MATE_TYPE_RR_SCREEN,
                          G_PARAM_WRITABLE | G_PARAM_CONSTRUCT_ONLY |
                              G_PARAM_STATIC_NICK | G_PARAM_STATIC_BLURB));
}

MateRRConfig *mate_rr_config_new_current(MateRRScreen *screen, GError **error) {
  MateRRConfig *self =
      g_object_new(MATE_TYPE_RR_CONFIG, "screen", screen, NULL);
...
static gboolean parse_file_gmarkup(const gchar *filename,
                                   const GMarkupParser *parser, gpointer data,
                                   GError **err) {
...
static gboolean output_match(MateRROutputInfo *output1,
                             MateRROutputInfo *output2) {
  g_assert(MATE_IS_RR_OUTPUT_INFO(output1));
  g_assert(MATE_IS_RR_OUTPUT_INFO(output2));

  if (strcmp(output1->priv->name, output2->priv->name) != 0) return FALSE;

  if (strcmp(output1->priv->vendor, output2->priv->vendor) != 0) return FALSE;

  if (output1->priv->product != output2->priv->product) return FALSE;

  if (output1->priv->serial != output2->priv->serial) return FALSE;

  if (output1->priv->connected != output2->priv->connected) return FALSE;

  return TRUE;
}

static gboolean output_equal(MateRROutputInfo *output1,
                             MateRROutputInfo *output2) {
  g_assert(MATE_IS_RR_OUTPUT_INFO(output1));
  g_assert(MATE_IS_RR_OUTPUT_INFO(output2));

  if (!output_match(output1, output2)) return FALSE;

  if (output1->priv->on != output2->priv->on) return FALSE;

  if (output1->priv->on) {
    if (output1->priv->width != output2->priv->width) return FALSE;

    if (output1->priv->height != output2->priv->height) return FALSE;

    if (output1->priv->rate != output2->priv->rate) return FALSE;

    if (output1->priv->x != output2->priv->x) return FALSE;

    if (output1->priv->y != output2->priv->y) return FALSE;

    if (output1->priv->rotation != output2->priv->rotation) return FALSE;
  }

  return TRUE;
}

static MateRROutputInfo *find_output(MateRRConfig *config, const char *name) {
  int i;

  for (i = 0; config->priv->outputs[i] != NULL; ++i) {
    MateRROutputInfo *output = config->priv->outputs[i];

    if (strcmp(name, output->priv->name) == 0) return output;
  }
 ...
 * setups"
 */
gboolean mate_rr_config_match(MateRRConfig *c1, MateRRConfig *c2) {
  int i;
  ...
  for (i = 0; c1->priv->outputs[i] != NULL; ++i) {
    ...
    if (!output2 || !output_match(output1, output2)) return FALSE;
  }
 ...
 */
gboolean mate_rr_config_equal(MateRRConfig *c1, MateRRConfig *c2) {
  int i;
  g_return_val_if_fail(MATE_IS_RR_CONFIG(c1), FALSE);
  g_return_val_if_fail(MATE_IS_RR_CONFIG(c2), FALSE);
  ...
  for (i = 0; c1->priv->outputs[i] != NULL; ++i) {
    ...
    output2 = find_output(c2, output1->priv->name);
    if (!output2 || !output_equal(output1, output2)) return FALSE;
  }

  return TRUE;
}

static MateRROutputInfo **make_outputs(MateRRConfig *config) {
  GPtrArray *outputs;
  ...
  for (i = 0; config->priv->outputs[i] != NULL; ++i) {
    ...
    *(new->priv) = *(old->priv);
    if (old->priv->name) new->priv->name = g_strdup(old->priv->name);
    if (old->priv->display_name)
      new->priv->display_name = g_strdup(old->priv->display_name);

    if (old->priv->on && !first_on) first_on = old;
...
gboolean mate_rr_config_applicable(MateRRConfig *configuration,
                                   MateRRScreen *screen, GError **error) {
  MateRROutputInfo **outputs;
...
static void ensure_config_directory(void) {
  g_mkdir_with_parents(g_get_user_config_dir(), 0700);
}

char *mate_rr_config_get_backup_filename(void) {
  ensure_config_directory();
  return g_build_filename(g_get_user_config_dir(), CONFIG_BACKUP_BASENAME,
                          NULL);
}

char *mate_rr_config_get_intended_filename(void) {
  ensure_config_directory();
  return g_build_filename(g_get_user_config_dir(), CONFIG_INTENDED_BASENAME,
                          NULL);
}

static const char *get_rotation_name(MateRRRotation r) {
  if (r & MATE_RR_ROTATION_0) return "normal";
  if (r & MATE_RR_ROTATION_90) return "left";
  if (r & MATE_RR_ROTATION_180) return "upside_down";
  if (r & MATE_RR_ROTATION_270) return "right";
...
static const char *get_reflect_x(MateRRRotation r) {
  return yes_no(r & MATE_RR_REFLECT_X);
}

static const char *get_reflect_y(MateRRRotation r) {
  return yes_no(r & MATE_RR_REFLECT_Y);
}

static void emit_configuration(MateRRConfig *config, GString *string) {
  int j;
  ...
  for (j = 0; config->priv->outputs[j] != NULL; ++j) {
    ...
    g_string_append_printf(string, "      <output name=\"%s\">\n",
                           output->priv->name);

    if (output->priv->connected && *output->priv->vendor != '\0') {
      g_string_append_printf(string, "          <vendor>%s</vendor>\n",
                             output->priv->vendor);
      g_string_append_printf(string, "          <product>0x%04x</product>\n",
                             output->priv->product);
      g_string_append_printf(string, "          <serial>0x%08x</serial>\n",
                             output->priv->serial);
    }
    ...
    if (output->priv->connected && output->priv->on) {
      ...
      g_string_append_printf(string, "          <rotation>%s</rotation>\n",
                             get_rotation_name(output->priv->rotation));
      g_string_append_printf(string, "          <reflect_x>%s</reflect_x>\n",
                             get_reflect_x(output->priv->rotation));
      g_string_append_printf(string, "          <reflect_y>%s</reflect_y>\n",
                             get_reflect_y(output->priv->rotation));
      g_string_append_printf(string, "          <primary>%s</primary>\n",
...
void mate_rr_config_sanitize(MateRRConfig *config) {
  int i;
...
gboolean mate_rr_config_ensure_primary(MateRRConfig *configuration) {
  ...
  for (i = 0; priv->outputs[i] != NULL; ++i) {
    ...
    if (top_left == NULL || (info->priv->x < top_left->priv->x &&
                             info->priv->y < top_left->priv->y)) {
      top_left = info;
    }
    if (laptop == NULL && _mate_rr_output_name_is_laptop(info->priv->name)) {
      laptop = info;
    }
  }
...
gboolean mate_rr_config_save(MateRRConfig *configuration, GError **error) {
  ...
  configurations =
      configurations_read_from_file(intended_filename, NULL); /* NULL-GError */
  ...
  rename(intended_filename,
         backup_filename); /* no error checking because the intended file may
                              not even exist */

  result = g_file_set_contents(intended_filename, output->str, -1, error);

  if (!result)
    rename(backup_filename, intended_filename); /* no error checking because the
                                                   backup may not even exist */
...
gboolean mate_rr_config_apply_with_time(MateRRConfig *config,
                                        MateRRScreen *screen, guint32 timestamp,
                                        GError **error) {
  ...
  for (i = 0; outputs[i] != NULL; i++) g_object_unref(outputs[i]);
  g_free(outputs);

  if (assignment) {
    if (crtc_assignment_apply(assignment, timestamp, error)) result = TRUE;
 ...
 * @timestamp: X server timestamp from the event that causes the screen
 * configuration to change (a user's button press, for example)
 ...
 * RANDR outputs.
 *
 * Typically, @filename is the result of mate_rr_config_get_intended_filename()
 * or mate_rr_config_get_backup_filename().
 *
 ...
 *
 * Returns: (array zero-terminated=1) (element-type MateDesktop.RROutputInfo)
 * (transfer none): the output configuration for this #MateRRConfig
 */
MateRROutputInfo **mate_rr_config_get_outputs(MateRRConfig *self) {
 ...
 */
gboolean mate_rr_config_get_clone(MateRRConfig *self) {
  g_return_val_if_fail(MATE_IS_RR_CONFIG(self), FALSE);

  return self->priv->clone;
}

void mate_rr_config_set_clone(MateRRConfig *self, gboolean clone) {
...
static gboolean can_clone(CrtcInfo *info, MateRROutput *output) {
  guint i;

  for (i = 0; i < info->outputs->len; ++i) {
    MateRROutput *clone = info->outputs->pdata[i];

    if (!mate_rr_output_can_clone(clone, output)) return FALSE;
  }

  return TRUE;
}

static gboolean crtc_assignment_assign(CrtcAssignment *assign, MateRRCrtc *crtc,
                                       MateRRMode *mode, int x, int y,
                                       MateRRRotation rotation,
                                       gboolean primary, MateRROutput *output,
                                       GError **error) {
  ...
  if (!mate_rr_output_supports_mode(output, mode)) {
    g_set_error(error, MATE_RR_ERROR, MATE_RR_ERROR_CRTC_ASSIGNMENT,
                _("output %s does not support mode %dx%d@%dHz"), output_name,
                mate_rr_mode_get_width(mode), mate_rr_mode_get_height(mode),
  ...
  if (info) {
    if (!(info->mode == mode && info->x == x && info->y == y &&
          info->rotation == rotation)) {
      g_set_error(
          error, MATE_RR_ERROR, MATE_RR_ERROR_CRTC_ASSIGNMENT,
          _("output %s does not have the same parameters as another cloned "
            "output:\n"
            "existing mode = %d, new mode = %d\n"
            "existing coordinates = (%d, %d), new coordinates = (%d, %d)\n"
            "existing rotation = %s, new rotation = %s"),
          output_name, mate_rr_mode_get_id(info->mode),
          mate_rr_mode_get_id(mode), info->x, info->y, x, y,
    ...
    if (!can_clone(info, output)) {
      g_set_error(error, MATE_RR_ERROR, MATE_RR_ERROR_CRTC_ASSIGNMENT,
                  _("cannot clone to output %s"), output_name);
      return FALSE;
    }
...
static void crtc_assignment_unassign(CrtcAssignment *assign, MateRRCrtc *crtc,
                                     MateRROutput *output) {
  CrtcInfo *info = g_hash_table_lookup(assign->info, crtc);
  ...
  if (info) {
    ...
    if (info->outputs->len == 0) g_hash_table_remove(assign->info, crtc);
  }
}
...
static void configure_crtc(gpointer key, gpointer value, gpointer data) {
  MateRRCrtc *crtc = key;
  CrtcInfo *info = value;
  ...
  if (!mate_rr_crtc_set_config_with_time(crtc, state->timestamp, info->x,
                                         info->y, info->mode, info->rotation,
                                         (MateRROutput **)info->outputs->pdata,
                                         info->outputs->len, state->error))
    state->has_error = TRUE;
}

static gboolean mode_is_rotated(CrtcInfo *info) {
...
static gboolean crtc_is_rotated(MateRRCrtc *crtc) {
  MateRRRotation r = mate_rr_crtc_get_current_rotation(crtc);

  if ((r & MATE_RR_ROTATION_270) || (r & MATE_RR_ROTATION_90)) {
    return TRUE;
  }
...
static void accumulate_error(GString *accumulated_error, GError *error) {
 ...
 */
static gboolean real_assign_crtcs(MateRRScreen *screen,
                                  MateRROutputInfo **outputs,
                                  CrtcAssignment *assignment, GError **error) {
  ...
  for (i = 0; crtcs[i] != NULL; ++i) {
    ...
    g_string_append_printf(accumulated_error, _("Trying modes for CRTC %d\n"),
                           crtc_id);
    ...
    for (pass = 0; pass < 2; ++pass) {
      MateRROutput *mate_rr_output =
          mate_rr_screen_get_output_by_name(screen, output->priv->name);
      ...
      for (j = 0; modes[j] != NULL; ++j) {
        ...
        g_string_append_printf(accumulated_error,
                               _("CRTC %d: trying mode %dx%d@%dHz with output "
                                 "at %dx%d@%dHz (pass %d)\n"),
                               crtc_id, mode_width, mode_height, mode_freq,
                               output->priv->width, output->priv->height,
                               output->priv->rate, pass);
        ...
        if (mode_width == output->priv->width &&
          ...
          if (crtc_assignment_assign(
                  assignment, crtc, modes[j], output->priv->x, output->priv->y,
                  output->priv->rotation, output->priv->primary, mate_rr_output,
                  &my_error)) {
            my_error = NULL;
...
out:
  ...
  if (success)
    ...
    if (tried_mode)
      ...
      g_set_error(error, MATE_RR_ERROR, MATE_RR_ERROR_CRTC_ASSIGNMENT,
                  _("none of the selected modes were compatible with the "
                    "possible modes:\n%s"),
                  str);
...
static void crtc_info_free(CrtcInfo *info) {
  g_ptr_array_free(info->outputs, TRUE);
  g_free(info);
}

static void get_required_virtual_size(CrtcAssignment *assign, int *width,
                                      int *height) {
...
static CrtcAssignment *crtc_assignment_new(MateRRScreen *screen,
  ...
  assignment->info = g_hash_table_new_full(g_direct_hash, g_direct_equal, NULL,
                                           (GFreeFunc)crtc_info_free);
  ...
  if (real_assign_crtcs(screen, outputs, assignment, error)) {
    ...
    mate_rr_screen_get_ranges(screen, &min_width, &max_width, &min_height,
                              &max_height);
...
static gboolean crtc_assignment_apply(CrtcAssignment *assign, guint32 timestamp,
  ...
  mate_rr_screen_get_ranges(assign->screen, &min_width, &max_width, &min_height,
                            &max_height);
  ...
  gdk_x11_display_grab(
      gdk_screen_get_display(assign->screen->priv->gdk_screen));
  ...
  for (i = 0; all_crtcs[i] != NULL; ++i) {
    ...
    if (mode) {
      ...
      if (x + w > width || y + h > height ||
          !g_hash_table_lookup(assign->info, crtc)) {
        if (!mate_rr_crtc_set_config_with_time(crtc, timestamp, 0, 0, NULL,
                                               MATE_RR_ROTATION_0, NULL, 0,
                                               error)) {
          success = FALSE;
          break;
        }
