 ...
 * License along with the Mate Desktop Library; see the file COPYING.LIB.  If
...
static void mate_rr_output_info_init(MateRROutputInfo *self) {
...
static void mate_rr_output_info_class_init(MateRROutputInfoClass *klass) {
 ...
 *
 * Returns: whether there is a CRTC assigned to this output (i.e. a signal is
 * being sent to it)
 */
...
void mate_rr_output_info_get_geometry(MateRROutputInfo *self, int *x, int *y,
  ...
  if (x) *x = self->priv->x;
  if (y) *y = self->priv->y;
  if (width) *width = self->priv->width;
  if (height) *height = self->priv->height;
}
