...
static MateDesktopItem *ditem_load(ReadBuf *rb, gboolean no_translations,
                                   GError **error);
static gboolean ditem_save(MateDesktopItem *item, const char *uri,
                           GError **error);

static void mate_desktop_item_set_location_gfile(MateDesktopItem *item,
                                                 GFile *file);

static MateDesktopItem *mate_desktop_item_new_from_gfile(
    GFile *file, MateDesktopItemLoadFlags flags, GError **error);
...
static int readbuf_getc(ReadBuf *rb) {
  ...
  if (rb->size == 0 || rb->size == rb->pos) {
    ...
    if (rb->stream == NULL)
      ...
      bytes_read = g_input_stream_read(G_INPUT_STREAM(rb->stream), rb->buf,
                                       READ_BUF_SIZE, NULL, NULL);
...
static ReadBuf *readbuf_open(GFile *file, GError **error) {
  GError *local_error;
  GFileInputStream *stream;
  char *uri;
  ReadBuf *rb;
  ...
  if (stream == NULL) {
    g_set_error(error,
                /* FIXME: better errors */
                MATE_DESKTOP_ITEM_ERROR, MATE_DESKTOP_ITEM_ERROR_CANNOT_OPEN,
                _("Error reading file '%s': %s"), uri, local_error->message);
...
static gboolean readbuf_rewind(ReadBuf *rb, GError **error) {
  GError *local_error;

  rb->eof = FALSE;
  rb->pos = 0;

  if (!rb->past_first_read) return TRUE;

  rb->size = 0;

  if (g_seekable_seek(G_SEEKABLE(rb->stream), 0, G_SEEK_SET, NULL, NULL))
    return TRUE;
  ...
  if (rb->stream == NULL) {
    g_set_error(
        error, MATE_DESKTOP_ITEM_ERROR, MATE_DESKTOP_ITEM_ERROR_CANNOT_OPEN,
        _("Error rewinding file '%s': %s"), rb->uri, local_error->message);
    g_error_free(local_error);
...
static void readbuf_close(ReadBuf *rb) {
  if (rb->stream != NULL) g_object_unref(rb->stream);
  if (rb->file != NULL) g_object_unref(rb->file);
  g_free(rb->uri);
  if (rb->buf_needs_free) g_free(rb->buf);
  g_free(rb);
}

static MateDesktopItemType type_from_string(const char *type) {
  if (!type) return MATE_DESKTOP_ITEM_TYPE_NULL;
  ...
  switch (type[0]) {
    case 'A':
      ...
      break;
    case 'L':
      if (!strcmp(type, "Link")) return MATE_DESKTOP_ITEM_TYPE_LINK;
      break;
    case 'F':
      if (!strcmp(type, "FSDevice")) return MATE_DESKTOP_ITEM_TYPE_FSDEVICE;
      break;
    case 'M':
      if (!strcmp(type, "MimeType")) return MATE_DESKTOP_ITEM_TYPE_MIME_TYPE;
      break;
    case 'D':
      if (!strcmp(type, "Directory")) return MATE_DESKTOP_ITEM_TYPE_DIRECTORY;
      break;
    case 'S':
 ...
 *
 * Creates a MateDesktopItem object. The reference count on the returned value
 * is set to '1'.
 *
...
MateDesktopItem *mate_desktop_item_new(void) {
  ...
  retval->main_hash = g_hash_table_new_full(
      g_str_hash, g_str_equal, (GDestroyNotify)g_free, (GDestroyNotify)g_free);

  /* These are guaranteed to be set */
  mate_desktop_item_set_string(retval, MATE_DESKTOP_ITEM_NAME,
                               /* Translators: the "name" mentioned
                                * here is the name of an application or
                                * a document */
                               _("No name"));
  mate_desktop_item_set_string(retval, MATE_DESKTOP_ITEM_ENCODING, "UTF-8");
  mate_desktop_item_set_string(retval, MATE_DESKTOP_ITEM_VERSION, "1.0");
...
static void read_sort_order(MateDesktopItem *item, GFile *dir) {
  GFile *child;
  char buf[BUFSIZ];
  GString *str;
  ReadBuf *rb;
  ...
  readbuf_close(rb);
  if (str != NULL) {
    mate_desktop_item_set_string(item, MATE_DESKTOP_ITEM_SORT_ORDER, str->str);
    g_string_free(str, TRUE);
  }
}

static MateDesktopItem *make_fake_directory(GFile *dir) {
  MateDesktopItem *item;
  GFile *child;

  item = mate_desktop_item_new();
  mate_desktop_item_set_entry_type(item, MATE_DESKTOP_ITEM_TYPE_DIRECTORY);
...
static MateDesktopItem *mate_desktop_item_new_from_gfile(
  ...
  info = g_file_query_info(
      file, G_FILE_ATTRIBUTE_STANDARD_TYPE "," G_FILE_ATTRIBUTE_TIME_MODIFIED,
  ...
  if (type != G_FILE_TYPE_REGULAR && type != G_FILE_TYPE_DIRECTORY) {
    ...
    g_set_error(error,
                /* FIXME: better errors */
                MATE_DESKTOP_ITEM_ERROR, MATE_DESKTOP_ITEM_ERROR_INVALID_TYPE,
                _("File '%s' is not a regular file or directory."), uri);
  ...
  if (type == G_FILE_TYPE_DIRECTORY) {
    ...
    child_info = g_file_query_info(child, G_FILE_ATTRIBUTE_TIME_MODIFIED,
...
MateDesktopItem *mate_desktop_item_new_from_string(
    const char *uri, const char *string, gssize length,
    MateDesktopItemLoadFlags flags, GError **error) {
  MateDesktopItem *retval;
  ReadBuf *rb;
...
static char *lookup_desktop_file_in_data_dir(const char *desktop_file,
                                             const char *data_dir) {
  char *path;
...
static char *file_from_basename(const char *basename) {
  ...
  for (i = 0; system_data_dirs[i]; i++) {
    if ((retval =
             lookup_desktop_file_in_data_dir(basename, system_data_dirs[i]))) {
      return retval;
    }
  }
  return NULL;
...
MateDesktopItem *mate_desktop_item_new_from_basename(
    const char *basename, MateDesktopItemLoadFlags flags, GError **error) {
  MateDesktopItem *retval;
  char *file;
  ...
  if (!(file = file_from_basename(basename))) {
    g_set_error(error, MATE_DESKTOP_ITEM_ERROR,
                MATE_DESKTOP_ITEM_ERROR_CANNOT_OPEN, _("Cannot find file '%s'"),
                basename);
    return NULL;
  }
 ...
 */
gboolean mate_desktop_item_save(MateDesktopItem *item, const char *under,
                                gboolean force, GError **error) {
  const char *uri;

  if (under == NULL && !force && !item->modified) return TRUE;
  ...
  if (!ditem_save(item, uri, error)) return FALSE;
 ...
 * @item: A desktop item
 *
 * Decreases the reference count of the specified item, and destroys the item if
...
static Section *find_section(MateDesktopItem *item, const char *section) {
  GList *li;
  Section *sec;

  if (section == NULL) return NULL;
  if (strcmp(section, "Desktop Entry") == 0) return NULL;

  for (li = item->sections; li != NULL; li = li->next) {
    sec = li->data;
    if (strcmp(sec->name, section) == 0) return sec;
  }
...
static Section *section_from_key(MateDesktopItem *item, const char *key) {
  char *p;
  char *name;
  Section *sec;
...
static const char *lookup(const MateDesktopItem *item, const char *key) {
  return g_hash_table_lookup(item->main_hash, key);
}

static const char *lookup_locale(const MateDesktopItem *item, const char *key,
                                 const char *locale) {
...
static const char *lookup_best_locale(const MateDesktopItem *item,
                                      const char *key) {
...
static void set(MateDesktopItem *item, const char *key, const char *value) {
  ...
  if (sec != NULL) {
    if (value != NULL) {
      ...
      GList *list = g_list_find_custom(sec->keys, key_basename(key),
                                       (GCompareFunc)strcmp);
      if (list != NULL) {
        g_free(list->data);
...
static void set_locale(MateDesktopItem *item, const char *key,
  ...
  if (locale == NULL || strcmp(locale, "C") == 0) {
     ...
     * before */
    if (g_list_find_custom(item->languages, locale, (GCompareFunc)strcmp) ==
        NULL)
      item->languages = g_list_prepend(item->languages, g_strdup(locale));
  }
}
...
static GSList *make_args(GList *files) {
  GSList *list = NULL;
  GList *li;
...
static void free_args(GSList *list) {
  GSList *li;
...
static char *escape_single_quotes(const char *s, gboolean in_single_quotes,
...
static char *convert_uri(GFile *file, ConversionType conversion) {
  char *retval = NULL;
...
typedef enum { ADDED_NONE = 0, ADDED_SINGLE, ADDED_ALL } AddedStatus;

static AddedStatus append_all_converted(GString *str, ConversionType conversion,
                                        GSList *args, gboolean in_single_quotes,
                                        gboolean in_double_quotes,
  ...
  for (l = args; l; l = l->next) {
    ...
    if (!(converted = convert_uri(l->data, conversion))) continue;

    g_string_append(str, " ");

    escaped =
        escape_single_quotes(converted, in_single_quotes, in_double_quotes);
...
static AddedStatus append_first_converted(GString *str,
  ...
  for (l = *arg_ptr; l; l = l->next) {
    if ((converted = convert_uri(l->data, conversion))) break;

    *arg_ptr = l->next;
  }

  if (!converted) return added_status;
...
static gboolean do_percent_subst(const MateDesktopItem *item, const char *arg,
                                 GString *str, gboolean in_single_quotes,
                                 gboolean in_double_quotes, GSList *args,
                                 GSList **arg_ptr, AddedStatus *added_status) {
  char *esc;
  const char *cs;
  ...
  switch (arg[1]) {
    ...
    case 'U':
      *added_status =
          append_all_converted(str, URI_TO_STRING, args, in_single_quotes,
                               in_double_quotes, *added_status);
      break;
    case 'F':
      *added_status =
          append_all_converted(str, URI_TO_LOCAL_PATH, args, in_single_quotes,
                               in_double_quotes, *added_status);
      break;
    case 'N':
      *added_status = append_all_converted(str, URI_TO_LOCAL_BASENAME, args,
                                           in_single_quotes, in_double_quotes,
                                           *added_status);
      break;
    case 'D':
      *added_status = append_all_converted(str, URI_TO_LOCAL_DIRNAME, args,
                                           in_single_quotes, in_double_quotes,
                                           *added_status);
      break;
    case 'f':
      *added_status = append_first_converted(str, URI_TO_LOCAL_PATH, arg_ptr,
                                             in_single_quotes, in_double_quotes,
                                             *added_status);
      break;
    case 'u':
      *added_status =
          append_first_converted(str, URI_TO_STRING, arg_ptr, in_single_quotes,
                                 in_double_quotes, *added_status);
      break;
    case 'd':
      *added_status = append_first_converted(str, URI_TO_LOCAL_DIRNAME, arg_ptr,
                                             in_single_quotes, in_double_quotes,
                                             *added_status);
      break;
    case 'n':
      *added_status = append_first_converted(str, URI_TO_LOCAL_BASENAME,
    ...
    case 'k':
      if (item->location != NULL) {
        esc = escape_single_quotes(item->location, in_single_quotes,
                                   in_double_quotes);
        g_string_append(str, esc);
        g_free(esc);
      }
    ...
    default:
      ...
      if (g_ascii_isdigit(arg[1])) g_string_append_c(str, '%');
      return FALSE;
  }
...
static char *expand_string(const MateDesktopItem *item, const char *s,
                           GSList *args, GSList **arg_ptr,
                           AddedStatus *added_status) {
  const char *p;
  ...
  for (p = s; *p != '\0'; p++) {
    if (escape) {
      ...
      if (do_percent_subst(item, p, gs, single_quot, double_quot, args, arg_ptr,
                           added_status)) {
        p++;
      }
    } else {
...
static char **make_spawn_environment_for_sn_context(
    SnLauncherContext *sn_context, char **envp) {
  char **retval;
  char **freeme;
  int i, j;
...
static gboolean startup_timeout(void *data) {
  ...
  if (min_timeout < 1000) min_timeout = 1000;
...
static void add_startup_timeout(GdkScreen *screen,
  ...
  if (data == NULL) {
    ...
    g_object_set_data_full(G_OBJECT(screen), "mate-startup-data", data,
  ...
  if (data->timeout_id == 0) {
    data->timeout_id = g_timeout_add_seconds(STARTUP_TIMEOUT_LENGTH_SEC,
                                             startup_timeout, data);
  }
}
#endif /* HAVE_STARTUP_NOTIFICATION */

static inline char *stringify_uris(GSList *args) {
  GString *str;
...
static inline char *stringify_files(GSList *args) {
  GString *str;
...
static char **make_environment_for_screen(GdkScreen *screen, char **envp) {
  GdkDisplay *display;
  char **retval;
  char **freeme;
  char *display_name;
...
static int ditem_execute(const MateDesktopItem *item, const char *exec,
                         GList *file_list, GdkScreen *screen, int workspace,
                         char **envp, gboolean launch_only_one,
  ...
  if (item->type == MATE_DESKTOP_ITEM_TYPE_APPLICATION) {
    ...
    if (working_dir && !g_file_test(working_dir, G_FILE_TEST_IS_DIR))
      working_dir = NULL;
  }

  if (working_dir == NULL && !use_current_dir) working_dir = g_get_home_dir();
  ...
  if (mate_desktop_item_get_boolean(item, MATE_DESKTOP_ITEM_TERMINAL)) {
    ...
    if (options != NULL) {
      g_shell_parse_argv(options, &term_argc, &term_argv, NULL /* error */);
      /* ignore errors */
    }
  ...
  startup_class = mate_desktop_item_get_string(item, "StartupWMClass");
  if (startup_class || mate_desktop_item_get_boolean(item, "StartupNotify")) {
    const char *name;
    const char *icon;

    sn_context = sn_launcher_context_new(
        sn_display, screen ? gdk_x11_screen_get_screen_number(screen)
                           : DefaultScreen(GDK_DISPLAY_XDISPLAY(gdkdisplay)));

    name = mate_desktop_item_get_localestring(item, MATE_DESKTOP_ITEM_NAME);
    ...
    if (icon != NULL) sn_launcher_context_set_icon_name(sn_context, icon);
  ...
  do {
    ...
    new_exec = expand_string(item, exec_locale, args, &arg_ptr, &added_status);
    ...
    if (!g_shell_parse_argv(new_exec, &temp_argc, &temp_argv, error)) {
    ...
    for (i = 0; i < term_argc; i++)
      vector_list = g_slist_append(vector_list, g_strdup(term_argv[i]));

    for (i = 0; i < temp_argc; i++)
      vector_list = g_slist_append(vector_list, g_strdup(temp_argv[i]));
    ...
    if (sn_context != NULL && !sn_launcher_context_get_initiated(sn_context)) {
      ...
      sn_launcher_context_set_binary_name(sn_context, real_argv[0]);
      ...
      sn_launcher_context_initiate(
          sn_context, g_get_prgname() ? g_get_prgname() : "unknown",
          real_argv[0], launch_time);
    ...
    if (!g_spawn_async(working_dir, real_argv, envp,
                       (do_not_reap_child ? G_SPAWN_DO_NOT_REAP_CHILD : 0) |
    ...
    if (arg_ptr != NULL) arg_ptr = arg_ptr->next;

    /* rinse, repeat until we run out of arguments (That
     * is if we were adding singles anyway) */
  } while (added_status == ADDED_SINGLE && arg_ptr != NULL && !launch_only_one);
  ...
  if (term_argv) g_strfreev(term_argv);
...
static gboolean strip_the_amp(char *exec) {
  size_t exec_len;
...
static int mate_desktop_item_launch_on_screen_with_env(
    const MateDesktopItem *item, GList *file_list,
    MateDesktopItemLaunchFlags flags, GdkScreen *screen, int workspace,
    char **envp, GError **error) {
  const char *exec;
  char *the_exec;
  int ret;
  ...
  if (item->type == MATE_DESKTOP_ITEM_TYPE_LINK) {
    ...
    if (!(url && url[0] != '\0')) url = exec;

    if (!(url && url[0] != '\0')) {
      g_set_error(error, MATE_DESKTOP_ITEM_ERROR,
                  MATE_DESKTOP_ITEM_ERROR_NO_URL, _("No URL to launch"));
      return -1;
    }

    retval = gtk_show_uri_on_window(NULL, url, GDK_CURRENT_TIME, error);
    return retval ? 0 : -1;
  }
 ...
 */
int mate_desktop_item_launch(const MateDesktopItem *item, GList *file_list,
                             MateDesktopItemLaunchFlags flags, GError **error) {
  return mate_desktop_item_launch_on_screen_with_env(item, file_list, flags,
                                                     NULL, -1, NULL, error);
}
 ...
 * @workspace: the workspace on which the app should be launched (-1 for
 * current)
 * @error: FIXME
 *
 * See mate_desktop_item_launch for a full description. This function
...
int mate_desktop_item_launch_on_screen(const MateDesktopItem *item,
                                       GList *file_list,
                                       MateDesktopItemLaunchFlags flags,
                                       GdkScreen *screen, int workspace,
                                       GError **error) {
  return mate_desktop_item_launch_on_screen_with_env(
      item, file_list, flags, screen, workspace, NULL, error);
}
...
int mate_desktop_item_drop_uri_list(const MateDesktopItem *item,
                                    ...
                                    MateDesktopItemLaunchFlags flags,
                                    GError **error) {
  return mate_desktop_item_drop_uri_list_with_env(item, uri_list, flags, NULL,
                                                  error);
}
...
int mate_desktop_item_drop_uri_list_with_env(const MateDesktopItem *item,
  ...
  ret = mate_desktop_item_launch_with_env(item, list, flags, envp, error);
...
gboolean mate_desktop_item_exists(const MateDesktopItem *item) {
  ...
  if (try_exec != NULL && !exec_exists(try_exec)) {
    return FALSE;
  }
 ...
 *
 * Returns: An enum value that specifies whether the item has changed since
 * being loaded.
 */
MateDesktopItemStatus mate_desktop_item_get_file_status(
  ...
  if (item->location == NULL) return MATE_DESKTOP_ITEM_DISAPPEARED;
  ...
  else if (((guint64)item->mtime) < g_file_info_get_attribute_uint64(
 ...
 */
char *mate_desktop_item_find_icon(GtkIconTheme *icon_theme, const char *icon,
                                  int desired_size, int flags) {
  GtkIconInfo *info;
  char *full = NULL;
  ...
  if (icon == NULL || strcmp(icon, "") == 0) {
    ...
    if (p && (strcmp(p, ".png") == 0 || strcmp(p, ".xpm") == 0 ||
              strcmp(p, ".svg") == 0)) {
      *p = 0;
    }
 ...
 *
 * Description:  This function goes and looks for the icon file.  If the icon
 * is not set as an absolute filename, this will look for it in the standard
 * places. If it can't find the icon, it will return %NULL
 *
 * Returns: A newly allocated string
 */
char *mate_desktop_item_get_icon(const MateDesktopItem *item,
  ...
  return mate_desktop_item_find_icon(icon_theme, icon, 48 /* desired_size */,
                                     0 /* flags */);
}
...
void mate_desktop_item_set_location(MateDesktopItem *item,
  ...
  if (item->location != NULL && location != NULL &&
...
void mate_desktop_item_set_string(MateDesktopItem *item, const char *attr,
                                  const char *value) {
  g_return_if_fail(item != NULL);
  g_return_if_fail(item->refcount > 0);
...
const char *mate_desktop_item_get_attr_locale(const MateDesktopItem *item,
  ...
  for (i = 0; langs_pointer[i] != NULL; i++) {
    ...
    if (value) return langs_pointer[i];
  }
...
GList *mate_desktop_item_get_languages(const MateDesktopItem *item,
  ...
  for (li = item->languages; li != NULL; li = li->next) {
    ...
    if (attr == NULL || lookup_locale(item, attr, language) != NULL) {
...
void mate_desktop_item_set_localestring(MateDesktopItem *item, const char *attr,
                                        const char *value) {
  g_return_if_fail(item != NULL);
  g_return_if_fail(item->refcount > 0);
  g_return_if_fail(attr != NULL);
...
void mate_desktop_item_set_strings(MateDesktopItem *item, const char *attr,
                                   char **strings) {
  char *str, *str2;
...
gboolean mate_desktop_item_get_boolean(const MateDesktopItem *item,
  ...
  return (value[0] == 'T' || value[0] == 't' || value[0] == 'Y' ||
          value[0] == 'y' || atoi(value) != 0);
}

void mate_desktop_item_set_boolean(MateDesktopItem *item, const char *attr,
                                   gboolean value) {
  g_return_if_fail(item != NULL);
  g_return_if_fail(item->refcount > 0);
  g_return_if_fail(attr != NULL);
...
static gboolean G_GNUC_CONST standard_is_boolean(const char *key) {
  static GHashTable *bools = NULL;

  if (bools == NULL) {
    bools = g_hash_table_new(g_str_hash, g_str_equal);
    g_hash_table_insert(bools, MATE_DESKTOP_ITEM_NO_DISPLAY,
                        MATE_DESKTOP_ITEM_NO_DISPLAY);
    g_hash_table_insert(bools, MATE_DESKTOP_ITEM_HIDDEN,
                        MATE_DESKTOP_ITEM_HIDDEN);
    g_hash_table_insert(bools, MATE_DESKTOP_ITEM_TERMINAL,
                        MATE_DESKTOP_ITEM_TERMINAL);
    g_hash_table_insert(bools, MATE_DESKTOP_ITEM_READ_ONLY,
...
static gboolean G_GNUC_CONST standard_is_strings(const char *key) {
  static GHashTable *strings = NULL;

  if (strings == NULL) {
    strings = g_hash_table_new(g_str_hash, g_str_equal);
    g_hash_table_insert(strings, MATE_DESKTOP_ITEM_FILE_PATTERN,
                        MATE_DESKTOP_ITEM_FILE_PATTERN);
    g_hash_table_insert(strings, MATE_DESKTOP_ITEM_ACTIONS,
                        MATE_DESKTOP_ITEM_ACTIONS);
    g_hash_table_insert(strings, MATE_DESKTOP_ITEM_MIME_TYPE,
                        MATE_DESKTOP_ITEM_MIME_TYPE);
    g_hash_table_insert(strings, MATE_DESKTOP_ITEM_PATTERNS,
                        MATE_DESKTOP_ITEM_PATTERNS);
    g_hash_table_insert(strings, MATE_DESKTOP_ITEM_SORT_ORDER,
...
static GHashTable *init_encodings(void) {
  ...
  insert_locales(encodings, "ISO-8859-1", "br", "ca", "da", "de", "en", "es",
                 "eu", "fi", "fr", "gl", "it", "nl", "wa", "no", "pt", "pt",
                 "sv", NULL);
  insert_locales(encodings, "ISO-8859-2", "cs", "hr", "hu", "pl", "ro", "sk",
                 "sl", "sq", "sr", NULL);
...
static const char *get_encoding_from_locale(const char *locale) {
  ...
  if (encodings == NULL) encodings = init_encodings();
...
static Encoding get_encoding(ReadBuf *rb) {
  gboolean old_kde = FALSE;
  char buf[BUFSIZ];
  gboolean all_valid_utf8 = TRUE;

  while (readbuf_gets(buf, sizeof(buf), rb) != NULL) {
    if (strncmp(MATE_DESKTOP_ITEM_ENCODING, buf,
  ...
  if (old_kde) return ENCODING_LEGACY_MIXED;
...
static char *decode_string(const char *value, Encoding encoding,
                           const char *locale) {
  char *retval = NULL;
  ...
  if (locale != NULL && encoding == ENCODING_LEGACY_MIXED) {
    ...
    char *utf8_string;
    if (char_encoding == NULL) return NULL;
    if (strcmp(char_encoding, "ASCII") == 0) {
      return decode_string_and_dup(value);
    }
    utf8_string =
        g_convert(value, -1, "UTF-8", char_encoding, NULL, NULL, NULL);
    ...
    if (!g_utf8_validate(value, -1, NULL)) /* invalid utf8, ignore this key */
      return NULL;
    return decode_string_and_dup(value);
  } else {
...
static void insert_key(MateDesktopItem *item, Section *cur_section,
                       Encoding encoding, const char *key, const char *value,
                       gboolean old_kde, gboolean no_translations) {
  char *k;
  char *val;
  /* we always store everything in UTF-8 */
  if (cur_section == NULL && strcmp(key, MATE_DESKTOP_ITEM_ENCODING) == 0) {
    k = g_strdup(key);
    val = g_strdup("UTF-8");
  } else {
...
static void setup_type(MateDesktopItem *item, const char *uri) {
  const char *type =
      g_hash_table_lookup(item->main_hash, MATE_DESKTOP_ITEM_TYPE);
  if (type == NULL && uri != NULL) {
    char *base = g_path_get_basename(uri);
    if (base != NULL && strcmp(base, ".directory") == 0) {
      /* This gotta be a directory */
      g_hash_table_replace(item->main_hash, g_strdup(MATE_DESKTOP_ITEM_TYPE),
                           g_strdup("Directory"));
      item->keys = g_list_prepend(item->keys, g_strdup(MATE_DESKTOP_ITEM_TYPE));
...
static char *try_english_key(MateDesktopItem *item, const char *key) {
  char *str;
...
static void sanitize(MateDesktopItem *item, const char *uri) {
  const char *type;
  ...
  if (lookup(item, MATE_DESKTOP_ITEM_NAME) == NULL) {
    ...
    if (name == NULL && uri != NULL) name = g_path_get_basename(uri);
    ...
    g_hash_table_replace(item->main_hash, g_strdup(MATE_DESKTOP_ITEM_NAME),
                         name);
    item->keys = g_list_prepend(item->keys, g_strdup(MATE_DESKTOP_ITEM_NAME));
  }
  if (lookup(item, MATE_DESKTOP_ITEM_ENCODING) == NULL) {
    /* We store everything in UTF-8 so write that down */
    g_hash_table_replace(item->main_hash, g_strdup(MATE_DESKTOP_ITEM_ENCODING),
                         g_strdup("UTF-8"));
    item->keys =
        g_list_prepend(item->keys, g_strdup(MATE_DESKTOP_ITEM_ENCODING));
  }
  if (lookup(item, MATE_DESKTOP_ITEM_VERSION) == NULL) {
    /* this is the version that we follow, so write it down */
    g_hash_table_replace(item->main_hash, g_strdup(MATE_DESKTOP_ITEM_VERSION),
                         g_strdup("1.0"));
    item->keys =
        g_list_prepend(item->keys, g_strdup(MATE_DESKTOP_ITEM_VERSION));
  }
}
...
static MateDesktopItem *ditem_load(ReadBuf *rb, gboolean no_translations,
  ...
  if (encoding == ENCODING_UNKNOWN) {
    ...
    g_set_error(error, MATE_DESKTOP_ITEM_ERROR,
                ...
                _("Unknown encoding of: %s"), rb->uri);
    readbuf_close(rb);
    return NULL;
  }
  ...
  while ((c = readbuf_getc(rb)) != EOF) {
    ...
    switch (state) {
      case OnSecHeader:
        if (c == ']' || OVERFLOW) {
          *next = '\0';
          ...
          if (cur_section != NULL && cur_section->keys != NULL) {
      ...
      case KeyDefOnKey:
        ...
        if (state == FirstBrace) break;

        if ((c == ' ' && state != KeyDefOnKey) || c == '\t') break;
      ...
      case KeyValue:
        if (OVERFLOW || c == '\n') {
          *next = '\0';
          ...
          insert_key(item, cur_section, encoding, key, CharBuffer, old_kde,
                     no_translations);
...
static void stream_printf(GFileOutputStream *stream, const char *format, ...)
    G_GNUC_PRINTF(2, 3);
...
static void stream_printf(GFileOutputStream *stream, const char *format, ...) {
  ...
  g_output_stream_write(G_OUTPUT_STREAM(stream), s, strlen(s), NULL, NULL);
  g_free(s);
}

static void dump_section(MateDesktopItem *item, GFileOutputStream *stream,
                         Section *section) {
  GList *li;
...
static gboolean ditem_save(MateDesktopItem *item, const char *uri,
  ...
  if (item->sections != NULL) stream_printf(stream, "\n");
  ...
  for (li = item->sections; li != NULL; li = li->next) {
    ...
    if (li->next != NULL) stream_printf(stream, "\n");
  }
...
static gpointer _mate_desktop_item_copy(gpointer boxed) {
  return mate_desktop_item_copy(boxed);
}

static void _mate_desktop_item_free(gpointer boxed) {
...
GType mate_desktop_item_get_type(void) {
  ...
  if (type == 0) {
    type = g_boxed_type_register_static(
        "MateDesktopItem", _mate_desktop_item_copy, _mate_desktop_item_free);
  }
...
GQuark mate_desktop_item_error_quark(void) {
  static GQuark q = 0;
  if (q == 0) q = g_quark_from_static_string("mate-desktop-item-error-quark");
