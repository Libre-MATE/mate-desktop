...
/* MateRRCrtc */
static MateRRCrtc *crtc_new(ScreenInfo *info, RRCrtc id);
static MateRRCrtc *crtc_copy(const MateRRCrtc *from);
static void crtc_free(MateRRCrtc *crtc);

#ifdef HAVE_RANDR
static gboolean crtc_initialize(MateRRCrtc *crtc, XRRScreenResources *res,
                                GError **error);
#endif

/* MateRROutput */
static MateRROutput *output_new(ScreenInfo *info, RROutput id);

#ifdef HAVE_RANDR
static gboolean output_initialize(MateRROutput *output, XRRScreenResources *res,
                                  GError **error);
#endif
...
/* MateRRMode */
static MateRRMode *mode_new(ScreenInfo *info, RRMode id);

#ifdef HAVE_RANDR
static void mode_initialize(MateRRMode *mode, XRRModeInfo *info);
#endif
...
static void mate_rr_screen_set_property(GObject *, guint, const GValue *,
                                        GParamSpec *);
static void mate_rr_screen_get_property(GObject *, guint, GValue *,
                                        GParamSpec *);
static gboolean mate_rr_screen_initable_init(GInitable *, GCancellable *,
                                             GError **);
...
G_DEFINE_TYPE_WITH_CODE(MateRRScreen, mate_rr_screen, G_TYPE_OBJECT,
                        G_ADD_PRIVATE(MateRRScreen) G_IMPLEMENT_INTERFACE(
                            G_TYPE_INITABLE,
                            mate_rr_screen_initable_iface_init))
...
/* Screen */
static MateRROutput *mate_rr_output_by_id(ScreenInfo *info, RROutput id) {
  MateRROutput **output;
...
static MateRRCrtc *crtc_by_id(ScreenInfo *info, RRCrtc id) {
  MateRRCrtc **crtc;
...
static MateRRMode *mode_by_id(ScreenInfo *info, RRMode id) {
  MateRRMode **mode;
...
static void screen_info_free(ScreenInfo *info) {
  MateRROutput **output;
  MateRRCrtc **crtc;
  MateRRMode **mode;
  ...
  if (info->outputs) {
    for (output = info->outputs; *output; ++output) output_free(*output);
    g_free(info->outputs);
  }

  if (info->crtcs) {
    for (crtc = info->crtcs; *crtc; ++crtc) crtc_free(*crtc);
    g_free(info->crtcs);
  }

  if (info->modes) {
    for (mode = info->modes; *mode; ++mode) mode_free(*mode);
    g_free(info->modes);
  }
...
static gboolean has_similar_mode(MateRROutput *output, MateRRMode *mode) {
  int i;
...
static void gather_clone_modes(ScreenInfo *info) {
  int i;
  ...
  for (i = 0; info->outputs[i] != NULL; ++i) {
    ...
    for (j = 0; output1->modes[j] != NULL; ++j) {
      ...
      if (valid) g_ptr_array_add(result, mode);
    }
  }
...
#ifdef HAVE_RANDR
static gboolean fill_screen_info_from_resources(ScreenInfo *info,
  ...
  /* Initialize */
  for (crtc = info->crtcs; *crtc; ++crtc) {
    if (!crtc_initialize(*crtc, resources, error)) return FALSE;
  }

  for (output = info->outputs; *output; ++output) {
    if (!output_initialize(*output, resources, error)) return FALSE;
  }
...
static gboolean fill_out_screen_info(Display *xdisplay, Window xroot,
                                     ScreenInfo *info, gboolean needs_reprobe,
                                     GError **error) {
#ifdef HAVE_RANDR
  XRRScreenResources *resources;
  GdkDisplay *display;
  ...
  if (resources) {
    if (!fill_screen_info_from_resources(info, resources, error)) return FALSE;
  } else {
    g_set_error(
        error, MATE_RR_ERROR, MATE_RR_ERROR_RANDR_ERROR,
        /* Translators: a CRTC is a CRT Controller (this is X terminology). */
        _("could not get the screen resources (CRTCs, outputs, modes)"));
    return FALSE;
  }

  /* Then update the screen size range.  We do this after
   * XRRGetScreenResources() so that the X server will already have an updated
   * view of the outputs.
   */
  ...
  if (needs_reprobe) {
    ...
    success = XRRGetScreenSizeRange(xdisplay, xroot, &(info->min_width),
                                    &(info->min_height), &(info->max_width),
                                    &(info->max_height));
    gdk_display_flush(display);
    if (gdk_x11_display_error_trap_pop(display)) {
      g_set_error(
          error, MATE_RR_ERROR, MATE_RR_ERROR_UNKNOWN,
    ...
    mate_rr_screen_get_ranges(info->screen, &(info->min_width),
                              &(info->max_width), &(info->min_height),
                              &(info->max_height));
  }
...
static ScreenInfo *screen_info_new(MateRRScreen *screen, gboolean needs_reprobe,
                                   GError **error) {
  ...
  if (fill_out_screen_info(priv->xdisplay, priv->xroot, info, needs_reprobe,
                           error)) {
    return info;
  } else {
...
static gboolean screen_update(MateRRScreen *screen, gboolean force_callback,
...
#ifdef HAVE_RANDR
  if (info->resources->configTimestamp !=
      screen->priv->info->resources->configTimestamp)
    changed = TRUE;
#endif
...
static GdkFilterReturn screen_on_event(GdkXEvent *xevent, GdkEvent *event,
                                       gpointer data) {
#ifdef HAVE_RANDR
  ...
  if (!e) return GDK_FILTER_CONTINUE;
...
static gboolean mate_rr_screen_initable_init(GInitable *initable,
                                             GCancellable *canc, GError **error)

{
  ...
  if (XRRQueryExtension(dpy, &event_base, &ignore)) {
    ...
    if (priv->rr_major_version < 1 ||
        (priv->rr_major_version == 1 && priv->rr_minor_version < 3)) {
    ...
    XRRSelectInput(priv->xdisplay, priv->xroot, RRScreenChangeNotifyMask);
    gdk_x11_register_standard_event_type(
        gdk_screen_get_display(priv->gdk_screen), event_base, RRNotify + 1);
...
void mate_rr_screen_initable_iface_init(GInitableIface *iface) {
  iface->init = mate_rr_screen_initable_init;
}

void mate_rr_screen_finalize(GObject *gobject) {
  MateRRScreen *screen = MATE_RR_SCREEN(gobject);

  gdk_window_remove_filter(screen->priv->gdk_root, screen_on_event, screen);

  if (screen->priv->info) screen_info_free(screen->priv->info);

  G_OBJECT_CLASS(mate_rr_screen_parent_class)->finalize(gobject);
}

void mate_rr_screen_set_property(GObject *gobject, guint property_id,
...
void mate_rr_screen_get_property(GObject *gobject, guint property_id,
...
void mate_rr_screen_class_init(MateRRScreenClass *klass) {
  ...
  g_object_class_install_property(
      gobject_class, SCREEN_PROP_GDK_SCREEN,
      g_param_spec_object(
          "gdk-screen", "GDK Screen",
          "The GDK Screen represented by this MateRRScreen", GDK_TYPE_SCREEN,
          G_PARAM_READWRITE | G_PARAM_CONSTRUCT_ONLY | G_PARAM_STATIC_STRINGS));

  screen_signals[SCREEN_CHANGED] =
      g_signal_new("changed", G_TYPE_FROM_CLASS(gobject_class),
                   G_SIGNAL_RUN_FIRST | G_SIGNAL_NO_RECURSE | G_SIGNAL_NO_HOOKS,
                   G_STRUCT_OFFSET(MateRRScreenClass, changed), NULL, NULL,
                   g_cclosure_marshal_VOID__VOID, G_TYPE_NONE, 0);
}

void mate_rr_screen_init(MateRRScreen *self) {
...
MateRRScreen *mate_rr_screen_new(GdkScreen *screen, GError **error) {
  ...
  return g_initable_new(MATE_TYPE_RR_SCREEN, NULL, error, "gdk-screen", screen,
                        NULL);
}

void mate_rr_screen_set_size(MateRRScreen *screen, int width, int height,
  ...
  display = gdk_display_get_default();
  gdk_x11_display_error_trap_push(display);
  XRRSetScreenSize(screen->priv->xdisplay, screen->priv->xroot, width, height,
                   mm_width, mm_height);
  gdk_x11_display_error_trap_pop_ignored(display);
#endif
}
 ...
 */
void mate_rr_screen_get_ranges(MateRRScreen *screen, int *min_width,
  ...
  if (min_width) *min_width = priv->info->min_width;

  if (max_width) *max_width = priv->info->max_width;

  if (min_height) *min_height = priv->info->min_height;

  if (max_height) *max_height = priv->info->max_height;
}

/**
 * mate_rr_screen_get_timestamps:
 * @screen: a #MateRRScreen
 * @change_timestamp_ret: (out): Location in which to store the timestamp at
 * which the RANDR configuration was last changed
 * @config_timestamp_ret: (out): Location in which to store the timestamp at
 * which the RANDR configuration was last obtained
 *
...
static gboolean force_timestamp_update(MateRRScreen *screen) {
#ifdef HAVE_RANDR
  MateRRScreenPrivate *priv = screen->priv;
  MateRRCrtc *crtc;
  ...
  current_info =
      XRRGetCrtcInfo(priv->xdisplay, priv->info->resources, crtc->id);

  if (current_info == NULL) goto out;

  display = gdk_display_get_default();
  gdk_x11_display_error_trap_push(display);
  status = XRRSetCrtcConfig(
      priv->xdisplay, priv->info->resources, crtc->id, current_info->timestamp,
      current_info->x, current_info->y, current_info->mode,
      current_info->rotation, current_info->outputs, current_info->noutput);

  XRRFreeCrtcInfo(current_info);

  gdk_display_flush(display);
  if (gdk_x11_display_error_trap_pop(display)) goto out;

  if (status == RRSetConfigSuccess) timestamp_updated = TRUE;
out:
...
gboolean mate_rr_screen_refresh(MateRRScreen *screen, GError **error) {
  ...
  force_timestamp_update(
      screen); /* this is to keep other clients from thinking that the X server
...
MateRRCrtc *mate_rr_screen_get_crtc_by_id(MateRRScreen *screen, guint32 id) {
  ...
  for (i = 0; crtcs[i] != NULL; ++i) {
    if (crtcs[i]->id == id) return crtcs[i];
  }
...
static guint8 *get_property(Display *dpy, RROutput output, Atom atom,
  ...
  XRRGetOutputProperty(dpy, output, atom, 0, 100, False, False, AnyPropertyType,
...
static guint8 *read_edid_data(MateRROutput *output, gsize *len) {
  ...
  result = get_property(DISPLAY(output), output->id, edid_atom, len);
...
static char *get_connector_type_string(MateRROutput *output) {
#ifdef HAVE_RANDR
  char *result;
  unsigned char *prop;
  int actual_format;
  ...
  if (XRRGetOutputProperty(DISPLAY(output), output->id,
                           output->info->screen->priv->connector_type_atom, 0,
                           100, False, False, AnyPropertyType, &actual_type,
                           &actual_format, &nitems, &bytes_after,
                           &prop) != Success)
    return NULL;

  if (!(actual_type == XA_ATOM && actual_format == 32 && nitems == 1)) goto out;
...
static gboolean output_initialize(MateRROutput *output, XRRScreenResources *res,
  ...
  for (i = 0; i < info->ncrtc; ++i) {
    ...
    if (crtc) g_ptr_array_add(a, crtc);
  }
  g_ptr_array_add(a, NULL);
  ...
  for (i = 0; i < info->nclone; ++i) {
    MateRROutput *mate_rr_output =
        mate_rr_output_by_id(output->info, info->clones[i]);

    if (mate_rr_output) g_ptr_array_add(a, mate_rr_output);
  }
  g_ptr_array_add(a, NULL);
  ...
  for (i = 0; i < info->nmode; ++i) {
    ...
    if (mode) g_ptr_array_add(a, mode);
  }
  g_ptr_array_add(a, NULL);
...
MateRROutput *mate_rr_screen_get_output_by_name(MateRRScreen *screen,
  ...
  for (i = 0; screen->priv->info->outputs[i] != NULL; ++i) {
    ...
    if (strcmp(output->name, name) == 0) return output;
  }
...
gboolean _mate_rr_output_name_is_laptop(const char *name) {
  ...
  if (strstr(name, "lvds") || /* Most drivers use an "LVDS" prefix... */
      ...
      strstr(name, "LCD") || /* ... but fglrx uses "LCD" in some versions. Shoot
                                me now, kthxbye. */
 ...
 * @y: (out) (allow-none):
 */
void mate_rr_output_get_position(MateRROutput *output, int *x, int *y) {
  MateRRCrtc *crtc;
...
MateRRMode *mate_rr_output_get_preferred_mode(MateRROutput *output) {
  g_return_val_if_fail(output != NULL, NULL);
  if (output->n_preferred) return output->modes[0];
...
gboolean mate_rr_output_supports_mode(MateRROutput *output, MateRRMode *mode) {
  int i;
  ...
  for (i = 0; output->modes[i] != NULL; ++i) {
    if (output->modes[i] == mode) return TRUE;
  }

  return FALSE;
}

gboolean mate_rr_output_can_clone(MateRROutput *output, MateRROutput *clone) {
  int i;
  ...
  for (i = 0; output->clones[i] != NULL; ++i) {
    if (output->clones[i] == clone) return TRUE;
  }
...
static MateRRRotation mate_rr_rotation_from_xrotation(Rotation r) {
  gsize i;
  MateRRRotation result = 0;

  for (i = 0; i < G_N_ELEMENTS(rotation_map); ++i) {
    if (r & rotation_map[i].xrot) result |= rotation_map[i].rot;
  }

  return result;
}

static Rotation xrotation_from_rotation(MateRRRotation r) {
  gsize i;
  Rotation result = 0;

  for (i = 0; i < G_N_ELEMENTS(rotation_map); ++i) {
    if (r & rotation_map[i].rot) result |= rotation_map[i].xrot;
  }
...
gboolean mate_rr_crtc_set_config(MateRRCrtc *crtc, int x, int y,
                                 MateRRMode *mode, MateRRRotation rotation,
                                 MateRROutput **outputs, int n_outputs,
                                 GError **error) {
  return mate_rr_crtc_set_config_with_time(crtc, GDK_CURRENT_TIME, x, y, mode,
                                           rotation, outputs, n_outputs, error);
}
#endif

gboolean mate_rr_crtc_set_config_with_time(MateRRCrtc *crtc, guint32 timestamp,
                                           int x, int y, MateRRMode *mode,
                                           MateRRRotation rotation,
                                           MateRROutput **outputs,
                                           int n_outputs, GError **error) {
#ifdef HAVE_RANDR
  ScreenInfo *info;
  GArray *output_ids;
  GdkDisplay *display;
  ...
  g_return_val_if_fail(mode != NULL || outputs == NULL || n_outputs == 0,
                       FALSE);
  g_return_val_if_fail(error == NULL || *error == NULL, FALSE);
...
gboolean mate_rr_crtc_can_drive_output(MateRRCrtc *crtc, MateRROutput *output) {
  int i;
  ...
  for (i = 0; crtc->possible_outputs[i] != NULL; ++i) {
    if (crtc->possible_outputs[i] == output) return TRUE;
  }
 ...
 * @y: (out) (allow-none):
 */
void mate_rr_crtc_get_position(MateRRCrtc *crtc, int *x, int *y) {
...
static gboolean crtc_initialize(MateRRCrtc *crtc, XRRScreenResources *res,
  ...
  if (!info) {
    ...
    /* FIXME: can we actually catch BadRRCrtc, and does it make sense to emit
     * that? */
     ...
     * only listed for completeness. */
    g_set_error(error, MATE_RR_ERROR, MATE_RR_ERROR_RANDR_ERROR,
                _("could not get information about CRTC %d"), (int)crtc->id);
    return FALSE;
  }
  ...
  for (i = 0; i < info->noutput; ++i) {
    ...
    if (output) g_ptr_array_add(a, output);
  }
  g_ptr_array_add(a, NULL);
...
void mate_rr_crtc_set_gamma(MateRRCrtc *crtc, int size, unsigned short *red,
  ...
  if (size != crtc->gamma_size) return;
 ...
 */
gboolean mate_rr_crtc_get_gamma(MateRRCrtc *crtc, int *size,
