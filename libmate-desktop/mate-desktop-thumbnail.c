...
static void thumbnailers_directory_changed(
    GFileMonitor *monitor, GFile *file, GFile *other_file,
    GFileMonitorEvent event_type, MateDesktopThumbnailFactory *factory);
...
G_DEFINE_TYPE_WITH_PRIVATE(MateDesktopThumbnailFactory,
                           mate_desktop_thumbnail_factory, G_TYPE_OBJECT)
...
static Thumbnailer *thumbnailer_ref(Thumbnailer *thumb) {
...
static void thumbnailer_unref(Thumbnailer *thumb) {
...
static Thumbnailer *thumbnailer_load(Thumbnailer *thumb) {
  ...
  thumb->command =
      g_key_file_get_string(key_file, THUMBNAILER_ENTRY_GROUP, "Exec", NULL);
  if (!thumb->command) {
  ...
  thumb->mime_types = g_key_file_get_string_list(
  ...
  thumb->try_exec =
      g_key_file_get_string(key_file, THUMBNAILER_ENTRY_GROUP, "TryExec", NULL);
...
static Thumbnailer *thumbnailer_reload(Thumbnailer *thumb) {
  g_return_val_if_fail(thumb != NULL, NULL);
...
static Thumbnailer *thumbnailer_new(const gchar *path) {
  Thumbnailer *thumb;
...
static gboolean thumbnailer_try_exec(Thumbnailer *thumb) {
  gchar *path;
  gboolean retval;

  if (G_UNLIKELY(!thumb)) return FALSE;

  /* TryExec is optional, but Exec isn't, so we assume
   * the thumbnailer can be run when TryExec is not present
   */
  if (!thumb->try_exec) return TRUE;
...
static gpointer init_thumbnailers_dirs(gpointer data) {
  ...
  g_ptr_array_add(thumbs_dirs, g_build_filename(g_get_user_data_dir(),
                                                "thumbnailers", NULL));
  for (i = 0; data_dirs[i] != NULL; i++)
    g_ptr_array_add(thumbs_dirs,
                    g_build_filename(data_dirs[i], "thumbnailers", NULL));
  g_ptr_array_add(thumbs_dirs, NULL);

  return g_ptr_array_free(thumbs_dirs, FALSE);
}

static const gchar *const *get_thumbnailers_dirs(void) {
  static GOnce once_init = G_ONCE_INIT;
  return g_once(&once_init, init_thumbnailers_dirs, NULL);
}

/* These should be called with the lock held */
static void mate_desktop_thumbnail_factory_register_mime_types(
  ...
  for (i = 0; thumb->mime_types[i]; i++) {
    if (!g_hash_table_lookup(priv->mime_types_map, thumb->mime_types[i]))
      g_hash_table_insert(priv->mime_types_map, g_strdup(thumb->mime_types[i]),
                          thumbnailer_ref(thumb));
  }
}

static void mate_desktop_thumbnail_factory_add_thumbnailer(
    MateDesktopThumbnailFactory *factory, Thumbnailer *thumb) {
  MateDesktopThumbnailFactoryPrivate *priv = factory->priv;
...
static gboolean mate_desktop_thumbnail_factory_is_disabled(
    MateDesktopThumbnailFactory *factory, const gchar *mime_type) {
  MateDesktopThumbnailFactoryPrivate *priv = factory->priv;
  guint i;

  if (priv->disabled) return TRUE;

  if (!priv->disabled_types) return FALSE;

  for (i = 0; priv->disabled_types[i]; i++) {
    if (g_strcmp0(priv->disabled_types[i], mime_type) == 0) return TRUE;
  }

  return FALSE;
}

static gboolean remove_thumbnailer_from_mime_type_map(gchar *key,
                                                      Thumbnailer *value,
                                                      gchar *path) {
  return (strcmp(value->path, path) == 0);
}

static void update_or_create_thumbnailer(MateDesktopThumbnailFactory *factory,
                                         const gchar *path) {
  MateDesktopThumbnailFactoryPrivate *priv = factory->priv;
  GList *l;
  Thumbnailer *thumb;
  ...
  for (l = priv->thumbnailers; l && !found; l = g_list_next(l)) {
    ...
    if (strcmp(thumb->path, path) == 0) {
      ...
      g_hash_table_foreach_remove(
          priv->mime_types_map, (GHRFunc)remove_thumbnailer_from_mime_type_map,
          (gpointer)path);
  ...
  if (!found) {
    ...
    if (thumb) mate_desktop_thumbnail_factory_add_thumbnailer(factory, thumb);
  }

  g_mutex_unlock(&priv->lock);
}

static void remove_thumbnailer(MateDesktopThumbnailFactory *factory,
                               const gchar *path) {
  MateDesktopThumbnailFactoryPrivate *priv = factory->priv;
  GList *l;
  Thumbnailer *thumb;
...
static void remove_thumbnailers_for_dir(MateDesktopThumbnailFactory *factory,
  ...
  g_signal_handlers_disconnect_by_func(monitor, thumbnailers_directory_changed,
                                       factory);

  g_mutex_unlock(&priv->lock);
}

static void mate_desktop_thumbnail_factory_load_thumbnailers_for_dir(
    MateDesktopThumbnailFactory *factory, const gchar *path) {
  MateDesktopThumbnailFactoryPrivate *priv = factory->priv;
  GDir *dir;
  GFile *dir_file;
  GFileMonitor *monitor;
  ...
  if (monitor) {
    g_signal_connect(monitor, "changed",
                     G_CALLBACK(thumbnailers_directory_changed), factory);
  ...
  while ((dirent = g_dir_read_name(dir))) {
    ...
    if (!g_str_has_suffix(dirent, THUMBNAILER_EXTENSION)) continue;
...
static void mate_desktop_thumbnail_factory_load_thumbnailers(
    MateDesktopThumbnailFactory *factory) {
  MateDesktopThumbnailFactoryPrivate *priv = factory->priv;
  const gchar *const *dirs;
  guint i;

  if (priv->loaded) return;
...
static void external_thumbnailers_disabled_all_changed_cb(
...
static void external_thumbnailers_disabled_changed_cb(
    GSettings *settings, const gchar *key,
    MateDesktopThumbnailFactory *factory) {
  MateDesktopThumbnailFactoryPrivate *priv = factory->priv;
...
static void mate_desktop_thumbnail_factory_init(
    MateDesktopThumbnailFactory *factory) {
...
static void mate_desktop_thumbnail_factory_finalize(GObject *object) {
  MateDesktopThumbnailFactory *factory;
  MateDesktopThumbnailFactoryPrivate *priv;
  ...
  if (priv->settings) {
    g_signal_handlers_disconnect_by_func(
        priv->settings, external_thumbnailers_disabled_all_changed_cb, factory);
    g_signal_handlers_disconnect_by_func(
        priv->settings, external_thumbnailers_disabled_changed_cb, factory);
...
static void mate_desktop_thumbnail_factory_class_init(
    MateDesktopThumbnailFactoryClass *class) {
...
static char *thumbnail_path(const char *uri, MateDesktopThumbnailSize size) {
  char *path, *file;

  file = thumbnail_filename(uri);
  path = g_build_filename(
      g_get_user_cache_dir(), "thumbnails",
      size == MATE_DESKTOP_THUMBNAIL_SIZE_LARGE ? "large" : "normal", file,
      NULL);
  g_free(file);
  return path;
}

static char *thumbnail_failed_path(const char *uri) {
  char *path, *file;

  file = thumbnail_filename(uri);
  /* XXX: appname is only used for failed thumbnails. Is this a mistake? */
  path = g_build_filename(g_get_user_cache_dir(), "thumbnails", "fail", appname,
                          file, NULL);
  g_free(file);
  return path;
}

static char *validate_thumbnail_path(char *path, const char *uri, time_t mtime,
                                     MateDesktopThumbnailSize size) {
  GdkPixbuf *pixbuf;

  pixbuf = gdk_pixbuf_new_from_file(path, NULL);
  if (pixbuf == NULL || !mate_desktop_thumbnail_is_valid(pixbuf, uri, mtime)) {
    g_free(path);
    return NULL;
  }
...
static char *lookup_thumbnail_path(const char *uri, time_t mtime,
                                   MateDesktopThumbnailSize size) {
  char *path = thumbnail_path(uri, size);
  return validate_thumbnail_path(path, uri, mtime, size);
}

static char *lookup_failed_thumbnail_path(const char *uri, time_t mtime,
                                          MateDesktopThumbnailSize size) {
 ...
 *
 * Return value: (transfer full): The absolute path of the thumbnail, or %NULL
 *if none exist.
 *
 * Since: 2.2
 **/
char *mate_desktop_thumbnail_factory_lookup(
    MateDesktopThumbnailFactory *factory, const char *uri, time_t mtime) {
  MateDesktopThumbnailFactoryPrivate *priv = factory->priv;
 ...
 * @mtime: the mtime of the file
 *
 * Returns TRUE if this MateDesktopThumbnailFactory can (at least try) to
 *thumbnail this file. Thumbnails or files with failed thumbnails won't be
 *thumbnailed.
 *
 * Usage of this function is threadsafe.
 *
...
gboolean mate_desktop_thumbnail_factory_can_thumbnail(
    ...
    const char *mime_type, time_t mtime) {
  gboolean have_script = FALSE;

  /* Don't thumbnail thumbnails */
  if (uri && strncmp(uri, "file:/", 6) == 0 &&
  ...
  if (uri && (have_script)) {
    return !mate_desktop_thumbnail_factory_has_valid_failed_thumbnail(
        factory, uri, mtime);
  }
...
static char *expand_thumbnailing_elem(const char *elem, const int size,
                                      const char *inuri, const char *outfile,
                                      gboolean *got_input,
                                      gboolean *got_output) {
  GString *str;
  const char *p, *last;
...
static char **expand_thumbnailing_script(const char *script, const int size,
                                         const char *inuri, const char *outfile,
                                         GError **error) {
  GPtrArray *array;
  char **script_elems;
  ...
  if (!g_shell_parse_argv(script, NULL, &script_elems, error)) return NULL;
  ...
  for (i = 0; script_elems[i] != NULL; i++) {
    ...
    expanded = expand_thumbnailing_elem(script_elems[i], size, inuri, outfile,
                                        &got_in, &got_out);
...
static GdkPixbuf *get_preview_thumbnail(const char *uri, int size) {
  GdkPixbuf *pixbuf;
  GFile *file;
  GFileInfo *file_info;
  ...
  file_info = g_file_query_info(file, G_FILE_ATTRIBUTE_PREVIEW_ICON,
                                G_FILE_QUERY_INFO_NONE, NULL, /* GCancellable */
  ...
  input_stream = g_loadable_icon_load(G_LOADABLE_ICON(object), 0, /* size */
  ...
  pixbuf = gdk_pixbuf_new_from_stream_at_scale(input_stream, size, size, TRUE,
                                               NULL, NULL);
 ...
 * Usage of this function is threadsafe.
 *
 * Return value: (transfer full): thumbnail pixbuf if thumbnailing succeeded,
 *%NULL otherwise.
 *
 * Since: 2.2
 **/
GdkPixbuf *mate_desktop_thumbnail_factory_generate_thumbnail(
  ...
  size = 128;
  if (factory->priv->size == MATE_DESKTOP_THUMBNAIL_SIZE_LARGE) size = 256;
  ...
  if (!mate_desktop_thumbnail_factory_is_disabled(factory, mime_type)) {
    ...
    thumb = g_hash_table_lookup(factory->priv->mime_types_map, mime_type);
    if (thumb) script = g_strdup(thumb->command);
  }
  g_mutex_unlock(&factory->priv->lock);
  ...
  if (script) {
    ...
    if (fd != -1) {
      ...
      expanded_script =
          expand_thumbnailing_script(script, size, uri, tmpname, &error);
...
static gboolean save_thumbnail(GdkPixbuf *pixbuf, char *path, const char *uri,
                               time_t mtime) {
  char *dirname;
  char *tmp_path = NULL;
  int tmp_fd;
  ...
  if (g_mkdir_with_parents(dirname, 0700) != 0) goto out;

  tmp_path = g_strconcat(path, ".XXXXXX", NULL);
  tmp_fd = g_mkstemp(tmp_path);

  if (tmp_fd == -1) goto out;
  close(tmp_fd);
  ...
  if (width != NULL && height != NULL)
    ret = gdk_pixbuf_save(
        pixbuf, tmp_path, "png", &error, "tEXt::Thumb::Image::Width", width,
        "tEXt::Thumb::Image::Height", height, "tEXt::Thumb::URI", uri,
        "tEXt::Thumb::MTime", mtime_str, "tEXt::Software",
        "MATE::ThumbnailFactory", NULL);
  else
    ret = gdk_pixbuf_save(pixbuf, tmp_path, "png", &error, "tEXt::Thumb::URI",
                          uri, "tEXt::Thumb::MTime", mtime_str,
                          "tEXt::Software", "MATE::ThumbnailFactory", NULL);
  g_free(mtime_str);
...
static GdkPixbuf *make_failed_thumbnail(void) {
  GdkPixbuf *pixbuf;
...
void mate_desktop_thumbnail_factory_save_thumbnail(
    MateDesktopThumbnailFactory *factory, GdkPixbuf *thumbnail, const char *uri,
 ...
 * Since: 2.2
 **/
gboolean mate_desktop_thumbnail_has_uri(GdkPixbuf *pixbuf, const char *uri) {
 ...
 * Since: 2.2
 **/
gboolean mate_desktop_thumbnail_is_valid(GdkPixbuf *pixbuf, const char *uri,
                                         time_t mtime) {
  const char *thumb_uri, *thumb_mtime_str;
  time_t thumb_mtime;

  thumb_uri = gdk_pixbuf_get_option(pixbuf, "tEXt::Thumb::URI");
  if (g_strcmp0(uri, thumb_uri) != 0) return FALSE;

  thumb_mtime_str = gdk_pixbuf_get_option(pixbuf, "tEXt::Thumb::MTime");
  if (!thumb_mtime_str) return FALSE;
  thumb_mtime = (time_t)g_ascii_strtoll(thumb_mtime_str, (gchar **)NULL, 10);
  if (mtime != thumb_mtime) return FALSE;
