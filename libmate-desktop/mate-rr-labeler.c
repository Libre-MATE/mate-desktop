...
static int get_current_desktop(GdkScreen *screen) {
  Display *display;
  Window win;
  ...
  XGetWindowProperty(display, win, current_desktop, 0, G_MAXLONG, False,
...
static gboolean get_work_area(MateRRLabeler *labeler, GdkRectangle *rect) {
  Atom workarea;
  Atom type;
  Window win;
  int format;
  ...
  rect->y = 0;
  rect->width =
      WidthOfScreen(gdk_x11_screen_get_xscreen(labeler->priv->screen));
  rect->height =
      HeightOfScreen(gdk_x11_screen_get_xscreen(labeler->priv->screen));

  if (workarea == None) return FALSE;

  win = XRootWindow(display, disp_screen);
  result = XGetWindowProperty(display, win, workarea, 0, max_len, False,
                              AnyPropertyType, &type, &format, &num, &leftovers,
                              &ret_workarea);
...
static GdkFilterReturn screen_xevent_filter(GdkXEvent *xevent, GdkEvent *event,
...
static void mate_rr_labeler_init(MateRRLabeler *labeler) {
  GdkWindow *gdkwindow;

  labeler->priv = mate_rr_labeler_get_instance_private(labeler);

  labeler->priv->workarea_atom = XInternAtom(
  ...
  gdk_window_set_events(
      gdkwindow, gdk_window_get_events(gdkwindow) | GDK_PROPERTY_CHANGE_MASK);
}

static void mate_rr_labeler_set_property(GObject *gobject, guint property_id,
                                         const GValue *value,
...
static GObject *mate_rr_labeler_constructor(
    GType type, guint n_construct_properties,
    GObjectConstructParam *construct_properties) {
  MateRRLabeler *self =
      (MateRRLabeler *)G_OBJECT_CLASS(mate_rr_labeler_parent_class)
...
static void mate_rr_labeler_class_init(MateRRLabelerClass *klass) {
  ...
  g_object_class_install_property(
      ...
      g_param_spec_object("config", "Configuration",
                          "RandR configuration to label", MATE_TYPE_RR_CONFIG,
...
static void make_palette(MateRRLabeler *labeler) {
...
static gboolean label_window_draw_event_cb(GtkWidget *widget, cairo_t *cr,
                                           gpointer data) {
  GdkRGBA *color;
  ...
  cairo_rectangle(cr, LABEL_WINDOW_EDGE_THICKNESS / 2.0,
  ...
  cairo_rectangle(cr, LABEL_WINDOW_EDGE_THICKNESS, LABEL_WINDOW_EDGE_THICKNESS,
...
static void position_window(MateRRLabeler *labeler, GtkWidget *window, int x,
                            int y) {
  ...
  monitor_num = gdk_display_get_monitor_at_point(
      gdk_screen_get_display(labeler->priv->screen), x, y);
...
static GtkWidget *create_label_window(MateRRLabeler *labeler,
  ...
  g_signal_connect(window, "draw", G_CALLBACK(label_window_draw_event_cb),
                   labeler);
  ...
  if (mate_rr_config_get_clone(labeler->priv->config)) {
    ...
    display_name = g_strdup_printf("<b>%s</b>\n<small>%s</small>",
                                   mate_rr_output_info_get_display_name(output),
                                   mate_rr_output_info_get_name(output));
  ...
  /* Should we center this at the top edge of the monitor, instead of using the
   * upper-left corner? */
...
static void create_label_windows(MateRRLabeler *labeler) {
  ...
  for (i = 0; i < labeler->priv->num_outputs; i++) {
    if (!created_window_for_clone &&
        mate_rr_output_info_is_active(outputs[i])) {
      labeler->priv->windows[i] =
          create_label_window(labeler, outputs[i], labeler->priv->palette + i);
 ...
 * monitor. This is useful when users are required to identify which monitor is
 * which, e.g. for for configuring multiple monitors. The labels will be shown
 * by default, use mate_rr_labeler_hide to hide them.
 *
...
void mate_rr_labeler_get_rgba_for_output(MateRRLabeler *labeler,
  ...
  g_warning(
      "trying to get the color for unknown MateOutputInfo %p; returning "
      "magenta!",
      output);
