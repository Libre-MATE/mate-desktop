...
static gboolean mate_hsv_button_press(GtkWidget *widget, GdkEventButton *event);
static gboolean mate_hsv_button_release(GtkWidget *widget,
                                        GdkEventButton *event);
static gboolean mate_hsv_motion(GtkWidget *widget, GdkEventMotion *event);
static gboolean mate_hsv_draw(GtkWidget *widget, cairo_t *cr);
static gboolean mate_hsv_grab_broken(GtkWidget *widget,
                                     GdkEventGrabBroken *event);
static gboolean mate_hsv_focus(GtkWidget *widget, GtkDirectionType direction);
...
static void mate_hsv_class_init(MateHSVClass *class) {
  ...
  hsv_signals[CHANGED] = g_signal_new(
      I_("changed"), G_OBJECT_CLASS_TYPE(gobject_class), G_SIGNAL_RUN_FIRST,
      G_STRUCT_OFFSET(MateHSVClass, changed), NULL, NULL, NULL, G_TYPE_NONE, 0);

  hsv_signals[MOVE] = g_signal_new(
      I_("move"), G_OBJECT_CLASS_TYPE(gobject_class),
      G_SIGNAL_RUN_LAST | G_SIGNAL_ACTION, G_STRUCT_OFFSET(MateHSVClass, move),
      NULL, NULL, NULL, G_TYPE_NONE, 1, GTK_TYPE_DIRECTION_TYPE);

  binding_set = gtk_binding_set_by_class(class);

  gtk_binding_entry_add_signal(binding_set, GDK_KEY_Up, 0, "move", 1,
                               G_TYPE_ENUM, GTK_DIR_UP);
  gtk_binding_entry_add_signal(binding_set, GDK_KEY_KP_Up, 0, "move", 1,
                               G_TYPE_ENUM, GTK_DIR_UP);
  gtk_binding_entry_add_signal(binding_set, GDK_KEY_Down, 0, "move", 1,
                               G_TYPE_ENUM, GTK_DIR_DOWN);
  gtk_binding_entry_add_signal(binding_set, GDK_KEY_KP_Down, 0, "move", 1,
                               G_TYPE_ENUM, GTK_DIR_DOWN);
  gtk_binding_entry_add_signal(binding_set, GDK_KEY_Right, 0, "move", 1,
                               G_TYPE_ENUM, GTK_DIR_RIGHT);
  gtk_binding_entry_add_signal(binding_set, GDK_KEY_KP_Right, 0, "move", 1,
                               G_TYPE_ENUM, GTK_DIR_RIGHT);
  gtk_binding_entry_add_signal(binding_set, GDK_KEY_Left, 0, "move", 1,
                               G_TYPE_ENUM, GTK_DIR_LEFT);
  gtk_binding_entry_add_signal(binding_set, GDK_KEY_KP_Left, 0, "move", 1,
                               G_TYPE_ENUM, GTK_DIR_LEFT);
}
...
static void mate_hsv_realize(GtkWidget *widget) {
  ...
  attr.event_mask |=
      (GDK_KEY_PRESS_MASK | GDK_BUTTON_PRESS_MASK | GDK_BUTTON_RELEASE_MASK |
...
static void hsv_to_rgb(gdouble *h, gdouble *s, gdouble *v) {
...
static void compute_sv(MateHSV *hsv, gdouble x, gdouble y, gdouble *s,
                       gdouble *v) {
  GtkWidget *widget = GTK_WIDGET(hsv);
...
static gboolean is_in_triangle(MateHSV *hsv, gdouble x, gdouble y) {
...
static double compute_v(MateHSV *hsv, gdouble x, gdouble y) {
  GtkWidget *widget = GTK_WIDGET(hsv);
  double center_x;
  double center_y;
  double dx, dy;
...
static void set_cross_grab(MateHSV *hsv, GdkDevice *device, guint32 time) {
  ...
  cursor = gdk_cursor_new_for_display(gtk_widget_get_display(GTK_WIDGET(hsv)),
                                      GDK_CROSSHAIR);
  gdk_seat_grab(gdk_device_get_seat(device), priv->window,
                GDK_SEAT_CAPABILITY_ALL_POINTING, FALSE, cursor, NULL, NULL,
                NULL);
...
static void paint_ring(MateHSV *hsv, cairo_t *cr) {
  ...
  for (yy = 0; yy < height; yy++) {
    ...
    for (xx = 0; xx < width; xx++) {
      ...
      dist = dx * dx + dy * dy;
      if (dist < ((inner - 1) * (inner - 1)) ||
          dist > ((outer + 1) * (outer + 1))) {
        *p++ = 0;
        continue;
      }
      ...
      *p++ = (((int)(r * 255.0) << 16) | ((int)(g * 255.0) << 8) |
              (int)(b * 255.0));
    }
  }
  ...
  cairo_new_path(cr);
  cairo_arc(cr, center_x, center_y, priv->size / 2. - priv->ring_width / 2., 0,
            2 * G_PI);
  cairo_stroke(cr);
...
static void get_color(gdouble h, gdouble s, gdouble v, gint *r, gint *g,
                      gint *b) {
  hsv_to_rgb(&h, &s, &v);
...
#define LERP(a, b, v1, v2, i)                                              \
  (((v2) - (v1) != 0) ? ((a) + ((b) - (a)) * ((i) - (v1)) / ((v2) - (v1))) \
                      : (a))
...
#define FOCUS_RADIUS 6
  ...
  if (draw_focus && !priv->focus_on_ring) {
    ...
    gtk_render_focus(context, cr, xx - FOCUS_RADIUS - focus_width - focus_pad,
...
static gboolean mate_hsv_draw(GtkWidget *widget, cairo_t *cr) {
  ...
  if (draw_focus && priv->focus_on_ring) {
    ...
    gtk_render_focus(context, cr, 0, 0, gtk_widget_get_allocated_width(widget),
 ...
 */
GtkWidget *mate_hsv_new(void) { return g_object_new(MATE_TYPE_HSV, NULL); }
 ...
 */
void mate_hsv_set_color(MateHSV *hsv, gdouble h, gdouble s, gdouble v) {
  MateHSVPrivate *priv;
 ...
 */
void mate_hsv_get_color(MateHSV *hsv, double *h, double *s, double *v) {
  MateHSVPrivate *priv;
...
void mate_hsv_get_metrics(MateHSV *hsv, gint *size, gint *ring_width) {
  ...
  if (ring_width) *ring_width = priv->ring_width;
}
