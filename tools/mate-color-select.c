...
static gboolean copy_color(GtkWidget *widget, GdkEvent *event,
  ...
  mate_color_selection_get_current_color(
      MATE_COLOR_SELECTION(color_dialog->colorsel), &color);
...
int main(int argc, char **argv) {
  ...
  mate_color_selection_set_has_palette(MATE_COLOR_SELECTION(color_selection),
                                       TRUE);
  ...
  gtk_dialog_add_action_widget(GTK_DIALOG(color_dialog), widget,
                               GTK_RESPONSE_ACCEPT);
  g_signal_connect(widget, "button-release-event", G_CALLBACK(copy_color),
                   color_dialog);
